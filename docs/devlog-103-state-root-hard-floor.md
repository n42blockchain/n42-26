...
`StateSink::apply_diff` now carries the committed block hash. Every 32 Twig
versions by default, the node deterministically ranks changed accounts using
`blake3(block_hash || address)` and samples eight of them. The rate is tunable:
...
at sidecar version 2 while the reference reader returns the correct post-state.
With `interval=2`, the probe detects it inside the configured N-block window,
latches unhealthy, and proves that a later matching sample does not self-heal.
