...
**状态：已完成 mac 4-node E2E（2026-06-12）。**

- **Step 0**：`git -C ../reth log -1 --oneline` 必须 reth main 合并版（alloy-evm 0.36），不降级 deps。
- fresh-genesis 4-node：`N42_TWIG=1 N42_TWIG_SNAPSHOT_INTERVAL=2 N42_ENABLE_MDNS=0`
  启动成功；genesis root 四节点一致为
  `0x3d722f7c3ab136688a8a07c3a2e7b917ee3c3bff5257d3e058b5df83019207f6`。
- 真实 tx 推进：`0x30c6111c61da697d441f2c196b1b8426518e2da923f99a7f933b047c71fb4e52`
  在 block `0x48` 成功，随后四节点 twig root 一致变为
  `0xcd436390d268b94d7b6cbcb66a870e6862a342a0e36590b02ef789d758f2f78d`。
- 崩溃恢复：validator-1 在 snapshot gap 中 `kill -9`，kill 前
  `version=531`、root `0xcd436390d268b94d7b6cbcb66a870e6862a342a0e36590b02ef789d758f2f78d`；
  当时 `twig.snapshot` 为 version 530，`twig.wal` 为 20 bytes。重启后 RPC 返回
  `version=531` 和同一 root，日志确认 `Twig restored from snapshot/WAL`。
- 真实 RPC proof：新增 ignored test
  `crates/n42-mobile-ffi/tests/twig_rpc_e2e.rs`，调用 `n42_twigProof` 后走
  `n42_verify_twig_account_proof`，live node 返回 0。
