...
### T20 — Gov5 57d5b0d P2P 更新与 runtime30（2026-08-04）

runtime29 使用逐文件完全相同的 126 文件 / 17,318,592,333 字节停止态副本启动，
六端从 94,423 精确推进到 94,435，Rust 产生两个 `5+5` 块且无 panic/fatal/error。
启动后的最新性硬门捕获到 Gov5 `main` 已由 `8e1d27efb...` 前进到
`57d5b0d293...`，因此节点在 `00:13:05Z` 干净停止，窗口不计时且数据不复用；
排除记录 SHA-256 为 `fbed1cf1...c0c68`。

新上游两次提交只修改 P2P：固定 peer 集合关闭 peer scoring、seen-message TTL
由默认值缩短为 30 秒，并修复 scoring 关闭时仍调用 `SetScoreParams` 导致订阅失败。
没有数据库、创世、区块编码或共识提交路径变化。集成候选
`94584a4ae5c0189f602d2b078569beaf149439db` 已推送；定向测试、完整
`go test ./...` 和完整 `go test -race ./...` 均 PASS。两个独立冷 GOCACHE
生产构建逐字节一致，Gov binary SHA-256 为
...
3 小时门已额外挂载两个 fail-closed 深度等待器：一个在里程碑 PASS 后
复读归档/QMDB、网络 quorum、905 数据和资源趋势，另一个从 94,460 开始
扫描完整三小时六生产者区间。等待器证据前缀现可配置，默认保持
runtime28 向后兼容；runtime30 显式使用正确前缀，生产者失败证据也跟随
显式输出路径。替换只涉及新增的只读等待器，六节点与正式计时控制器
未重启。

phase guardian 也已扩展为可选监督里程碑等待器：在证据尚未产生时校验
精确 PID/命令，等待器结束后则要求输出 `status=PASS`，且失败证据全程
必须为空。runtime30 阶段守护已以 PID 17470 重挂，并将 3 小时两个深度
等待器纳入 fail-closed 管理；只替换该阶段守护，六节点、正式 monitor、
finalizer 和计时均连续。
...
100 分钟检查点继续严格 PASS：200 个 head 样本覆盖 6,034 秒、增长 624 块、
最大 gap 31 秒、lag 1、零交易；11 次 Gov main 采样精确，Rust `5+5` commit
...
正式零交易窗口从 `2026-08-04T04:20:54Z`、共同高度 94,537 开始，24 小时硬边界
为 `2026-08-05T04:20:54Z`。head/resource/Gov-main/官方 Reth stable 四条流和全部
finalizer、独立复验、rollover、总目标、边界、网络、六生产者、最终 905 数据与
completion V2 控制器均已挂载，phase guardian 首次快照 PASS。正式启动证据
SHA-256 为 `1d2da07fa4571467ca6f0a0b76b57ba1c075340ac9fea44c465694043266ff7e`。

10 分钟复合门严格 PASS：22 个 head 样本覆盖 637 秒，高度 94,537–94,603、增长
66 块、最大 lag 0、全程零交易；五个同 Rust PID 资源样本和两个 Gov-main 样本
均精确。完整六生产者扫描覆盖 94,538–94,603 的 66 个连续块和 11 轮，Rust 与
Gov1–Gov5 各生产 11 块，六端序列 SHA-256 均为
`15bfa039e4a47f7f051f7261cad096c5ba19d6f40fc5f0ec19a0cb01554d4485`。
同一边界的归档/QMDB、网络、905 数据和资源趋势复验全部 PASS；RSS 早期 24 小时
线性投影 916,443 KiB，低于 1,048,576 KiB 硬限并继续长期采样。补充证据 SHA-256
为 `2afa2fa1498ac6dc74d61a74479c8cb5e439d3ce3792de6d3142bc8097d8542e`（归档/QMDB）
及 `28b84fc12680e3463a1b290129040b9b00b288b88e2477a34e1911a935bc206a`（网络矩阵）。

---

### T24 — runtime37 告警洪泛排除与 runtime38 从零复测（2026-08-06）

runtime37 的最终冻结日志出现 9,426 条每 50 ms 重复的缺失 tx-forward leader 告警，
因此即使其链和生命周期检查通过，也不计入最终资格。排除证据 SHA-256 为
`c1f6d72cfe323dfd6273e4e4aa5d6d60fd33b0f049c163dd5d3b3288e4c0e289`。
N42 提交 `40d1ce69bd9a2102e51e96a77b2348a6bec915ea` 已将告警限制为每个共识 view
最多一次并推送，替换 Rust binary SHA-256 为
`e3ce3278e9be89418f726f41f9d1f0814cfddba7d869407f30d3ec39a519f533`。

runtime38 未复用 runtime37 推进后的数据，而是从干净停止的 runtime34 精确复制
141 文件 / 17,325,704,613 字节；复制清单 SHA-256 为
`ae4e7ff351afa4e010751fe2a79fa25ae5921a1fcdecf3d9c6ad695d1fe39a85`。
创世、92,605 的 905 边界、99,895 的安全边界、nonce `0x11`、六端同块、五个认证
Gov peer、quorum 5/4、direct push 5、CommitQC、零双签和归档/QMDB 均在新数据上
重新 PASS。

正式窗口从 `2026-08-06T12:41:42Z`、高度 99,925 从零开始。一小时门覆盖 3,635 秒、
121 个样本、增长 384 块、最大 lag 1、全程零交易。Rust 对齐区间
99,926--100,309 共 384 块和 64 轮，Rust 与 Gov1--Gov5 各生产 64 块，六端序列、
父链和槽位全部精确。冻结日志中的 73 个 Rust `5+5` commit 和 73 个 timeout-next-view
恢复全部精确，589 条 warning 全量归类，unknown/critical 均为 0。两个控制器边界
...
`e3ce3278e9be89418f726f41f9d1f0814cfddba7d869407f30d3ec39a519f533`。
canonical N42 锁文件的独立扫描覆盖 1,290 个依赖，同样为 0 漏洞；7 个允许 warning
中的 `lru 0.12.5` 只存在于未启用的 SP1 v4 证明图，当前 N42 与 Reth 运行依赖分别
解析为不受影响的 `lru 0.18.2` 和 `0.16.4`。

runtime38 的严格 6 小时门已于 `2026-08-06T18:42:36Z` 无放宽通过。正式快照覆盖
21,628 秒、715 个 head 样本，高度 99,925--102,229、增长 2,304 块、最大 lag 1、
交易 0；73 个同一 Rust PID 的资源样本覆盖 21,609 秒，RSS 峰值 255,984 KiB、线程
峰值 162、FD 峰值 90，24 小时 RSS 投影 315,982 KiB。Gov5 main 的 37 个上游样本
全部精确为 `c0a146468...`。里程碑和补充审计 SHA-256 分别为
`5f28ab642d9f634d82c4bb8f0e4993dc77f4951f9908cea60bf030c42be32c00` 与
`31600ff7d56472b730d172af41ebcb06fe9c2345828c696974508f890d04124d`；905 数据审计
再次确认创世和 92,605 边界六端精确、nonce `0x11`、txindex/QMDB opt-in 缺席且
无需重复制或重生成。

同一闭合边界的完整轮值审计扫描 99,926--102,229 共 2,304 块和 384 轮，Rust 与
Gov1--Gov5 各生产 384 块，六端序列、父链、槽位与零交易全部精确，证据 SHA-256
为 `450148f74fc0057dfff62c7ed200e115d3fd9dd4087c02c94363bcf37e886ad5`。V2 deep
审计扫描 99,902--102,230 共 2,329 块及 389 个 Rust 槽位；全部 Rust commit 为
有序 `5+5`，389 次 timeout 均在下一 view 恢复、pending 0，3,135 条 warning
全量归类且 missing-leader/unexpected/critical 均为 0。24 个 Gov 静态文件、复制
清单、二进制和冻结工具哈希全部精确；deep 证据 SHA-256 为
`50cf223598ce0728c9a160ac39f2fff26dbb5214e89979fb12ea3fb8cf0211d6`。节点未重启、
数据未改写、交易未发送；12/24 小时门及最终 burst/restart 生命周期继续运行。

12 小时严格门已于 `2026-08-07T00:43:33Z` 通过：1,430 个闭合 head 样本覆盖
43,294 秒，99,925--104,617 增长 4,692 块，最大采样间隔 31 秒、最大 lag 1，零交易
持续成立。145 个同 Rust PID 的资源样本覆盖 43,218 秒，RSS 峰值 267,104 KiB、线程
峰值 162、FD 峰值 90；73 个 Gov-main 样本均精确固定在 `c0a146468...`，CommitQC
为七验证者、双签为 0。里程碑 SHA-256 为
`f3168b44a757755f287b61e74a16798640505b7098cee54f0ede7ba2f17bfebd`。

独立冻结日志审计于高度 104,618 闭合，扫描 4,717 个连续父链块和 787 个 Rust 槽位；
六端均精确，每个槽位均为有序 `5+5` commit，view 步长和哈希顺序精确。787 个
timeout/pacemaker 对均在下一 view 恢复；6,344 条 warning 全量分区，missing-leader、
unexpected 和 critical 均为 0。审计未修改链状态，SHA-256 为
`9f9569fab20f210db135188a05b035aaca5cd788e35d91ca07d9b0e15eca7bd0`。24 小时零交易
窗口仍在运行；仅在其严格闭合后才允许发送已签名 burst 并进行 Rust 重启重加入验证。
