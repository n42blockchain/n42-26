...
### T7 — H2-v4 批量验签（本机代码侧 / 非阻塞）

`ConsensusSigningProfile::H2V4` 目前退化为逐签名 `verify_single`，Native 的
`batch_verify_with_fallback`（devlog-101，500 节点 QC 351.0ms→137.3ms）在 H2-v4 下
不生效。7 节点混合委员会无影响，**委员会规模或 Rust 占比提升前必须补齐**。

**验收**：H2-v4 走批量验签且坏签名能精确回退到具体 signer；跨语言向量不变。
