...
`ShardedTwig` 加 snapshot/restore（append-slot 历史 = entries 切片，完全决定 root）：
- `snapshot() -> TwigSnapshot`（version + 16 shard 的 entry log，serde 可序列化）。
- `from_snapshot(&TwigSnapshot)`（按 slot 顺序 replay 每个 active entry 的叶 + 重建 index/root）。
- `EntrySnapshot/ShardSnapshot/TwigSnapshot`（serde derive，供 bincode 落盘）。

测试 `snapshot_roundtrip_preserves_root_and_state`：4000 插入 + 500 更新（造死 slot）+ 500 删除，
snapshot → **bincode 序列化 → 反序列化 → from_snapshot** → root 字节一致、get（含更新/删除）一致、
proof 仍验证通过。14 测试全过，clippy 干净。
