...
3. **每块一次 fsync + 第二次快照写**（低-中）：`advance_execution_validated_head`
   ...
   规矩量一次每块 persist 耗时。
4. **eager-import 水位语义放宽**（信息）：为修 catch-up 毒化，抑制从"即写即拦"
   改为"Valid 后才抬水位"，且视图切换时重置为 0。同高度不同哈希的两个 eager
   import 现在可能双双提交给 reth。这是修复必须付的代价，但属原生生产路径的行为
   放宽，E2E 场景 4/9 的视图切换与恢复用例值得盯一轮。
...
- E2E 由 CI 验证；建议合入 main 后额外手动跑一轮场景 9（recovery）覆盖上面第 4 项。
