...
1. Translate revm selfdestructed accounts to `AccountWrite::Destroyed` and do
   not publish their per-slot writes. Translate revm `Created` accounts to an
   explicit `Recreated` write, because creation also hides every unmaterialized
   base-state slot even when destroy/recreate happens inside one transaction.
...
- recreated address hides an unread base-state slot without enumerating it;
