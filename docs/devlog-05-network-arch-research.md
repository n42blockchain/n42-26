...
> 回顾日期：2026-02-20
> 覆盖范围：Phase 1（日志-06）、Phase 2（日志-07）、Phase 3（日志-08）全部实现

### 11.1 五个瓶颈解决状态

| # | 瓶颈 | 原始影响 | 状态 | 解决方式 |
|---|------|---------|------|---------|
| 1 | `broadcast::channel` Clone 开销 | 10K×100KB = 1GB/块 | ✅ 已消除 | Phase 1: Bytes 零拷贝 → Phase 3: 移除 broadcast channel，改 per-session mpsc + try_send |
| 2 | Per-Phone Tokio Task (50K+ 调度压力) | task 数线性增长 | ⚠️ 缓解 | timeout + skip 防阻塞；ShardedStarHub 分散 runtime 压力。仍为 1 task/phone |
| 3 | 逐连接 QUIC stream 创建 | 10K × open_uni/块 | ⚠️ 缓解 | Phase 2 预分帧减半 syscall (2→1 write_all)。仍为 per-phone per-block 新 stream |
| 4 | CacheSync 广播浪费 | 新连接→所有手机 | ✅ 已解决 | Phase 1: phone_connected_tx + send_to_session 定向发送 |
| 5 | 无压缩 | 100KB 原始传输 | ✅ 已解决 | Phase 1: zstd level 3。实测 witness 压缩率 92-97%（远超预估 60-80%）|

### 11.2 七层优化方案完成对照

| 层 | 研究建议 | 状态 | 实现阶段 | 压测验证 |
|----|---------|------|---------|---------|
| 1 | Vec\<u8\> → Bytes 零拷贝 | ✅ | Phase 1 | 500KB clone: 946x 加速 |
| 2 | 3s 发送超时 + 慢连接跳过 | ✅ | Phase 1 | 超时→跳过不断开，metrics 可观测 |
| 3 | zstd 压缩 | ✅ | Phase 1 | 123KB→4KB (3%)，吞吐 1685 MB/s |
| 4 | 定向 CacheSync | ✅ | Phase 1 | phone_connected_tx 通道通知 |
| 5 | 分片 + FEC 广播 | ❌ 未实现 | — | 研究折中方案（QUIC stream 流式分片）亦未做 |
| 6 | 多 Endpoint 分片 | ✅ | Phase 2 | ShardedStarHub，不同端口（非 SO_REUSEPORT） |
| 7 | 连接分级 + 优先级 | ✅ | Phase 2+3 | PhoneTier + EWMA RTT + 分级广播 |

### 11.3 超出原计划的额外实现

| Feature | 阶段 | 说明 |
|---------|------|------|
| 预分帧 preframe_message | Phase 2 | 合并 type prefix + payload 为单次 write_all，研究文档未专门提出 |
| EWMA RTT (CAS 无锁) | Phase 3 | 替代累积平均，alpha=0.3，响应灵敏。研究只提"记录延迟" |
| Arc\<MobileSession\> 免锁 | Phase 3 | phone handler 持有本地 Arc，不再查 HashMap。压测 5.1x 加速 |
| Attestation 延迟指标 | Phase 3 | 首 receipt → 2/3 阈值的 histogram。研究未涉及 |
| 对称 tier metrics | Phase 2 fixup | broadcast 成功路径也上报 tier 标签 |
...
**Phase 2（架构改进）— ⚠️ 2/3 完成**

| 计划项 | 状态 | 说明 |
|--------|------|------|
| 多 Endpoint 分片 | ✅ | ShardedStarHub，N42_STARHUB_SHARDS 配置 |
| 连接分级 | ✅ | Fast/Normal/Slow 三级 + 分级广播 |
| 流式分片发送 | ❌ | 大包分 chunk 流式写入，减少首字节延迟 |

**Phase 3（深度优化）— ❌ 0/3 未开始**

| 计划项 | 状态 | 阻塞 |
|--------|------|------|
| QUIC Datagram + FEC | ❌ | quinn RFC 9221 API 未稳定 |
| 专用发送线程池 | ❌ | 需 benchmark 证明瓶颈在 syscall |
| 预分配连接池 | ❌ | 架构改动大，当前 HashMap 性能已足够 |

---

## 12. 压测数据（Release Mode）

> 测试文件：`crates/n42-node/tests/comm_stress_bench.rs`
> 运行命令：`cargo test -p n42-node --test comm_stress_bench --release -- --nocapture`

### 12.1 Phase 1 — 零拷贝

| 数据大小 | Vec::clone | Bytes::clone | 加速比 |
|----------|-----------|-------------|--------|
| 1 KB | 0.05 us | 0.01 us | 5x |
| 10 KB | 0.20 us | 0.01 us | 19x |
| 100 KB | 2.5 us | 0.01 us | **254x** |
| 500 KB | 9.2 us | 0.01 us | **946x** |

在 10K 手机广播 100KB 包场景下，零拷贝节省 `2.5us × 10K = 25ms`。

### 12.2 Phase 1 — zstd 压缩

| 数据规模 | 原始大小 | 压缩后 | 压缩率 | 压缩吞吐 | 解压吞吐 |
|---------|---------|--------|--------|---------|---------|
| 100 accounts | 8 KB | ~0.7 KB | 8% | 556 MB/s | 111 MB/s |
| 500 accounts | 41 KB | ~2.5 KB | 6% | 1027 MB/s | 139 MB/s |
| 1500 accounts | 123 KB | ~4 KB | **3%** | 1685 MB/s | 123 MB/s |

关键发现：witness 数据（零填充地址 + storage key + 重复 value 模式）的压缩率远超预期。100KB 包压缩后仅 3-4KB，这从根本上改变了带宽估算。

### 12.3 Phase 2 — 预分帧

| 数据大小 | 单次耗时 | 吞吐 |
|----------|---------|------|
| 10 KB | 0.2 us | 54.7 GB/s |
| 100 KB | 2.4 us | 41.0 GB/s |
| 500 KB | 9.3 us | 53.9 GB/s |

预分帧开销可忽略（内存拷贝级），但每次广播节省 N 次 syscall（write type prefix + write data → 单次 write）。

### 12.4 Phase 3 — EWMA RTT

| 场景 | 吞吐量 |
|------|--------|
| 单线程 | 230.9M ops/s |
| 8 线程 CAS 竞争 | 14.5M ops/s |

50K 手机每 8s slot 仅需 50K 次 RTT 更新，14.5M ops/s 的吞吐量有 290x 余量。

### 12.5 Phase 3 — Tier 排序

| 规模 | 排序耗时 |
|------|---------|
| 1K sessions | 0.003 ms |
| 10K sessions | 0.03 ms |
| 50K sessions | **0.14 ms** |

50K 规模排序仅 0.14ms，研究阶段标注"可能需要优化"实际完全不需要。

### 12.6 Phase 3 — Arc 直接访问 vs HashMap 查找

| 模式 | 10K sessions 吞吐 | 加速比 |
|------|-------------------|--------|
| HashMap lookup (pre-Phase3) | 30.5M ops/s | baseline |
| Arc direct access (Phase 3) | — | **5.1x faster** |

### 12.7 Phase 3 — Channel Fan-out
...
### 12.8 全管线端到端（compress + preframe + sort + fan-out）

| 规模 | 管线耗时 | 占 500ms 预算 | 占 8s slot |
|------|---------|-------------|-----------|
| 1K phones | 0.04 ms | 0.01% | 0.001% |
| 10K phones | 0.93 ms | 0.19% | 0.01% |
| 50K phones | **8.41 ms** | **1.7%** | **0.1%** |

**结论：50K 手机全管线 CPU 开销仅 8.4ms，占 8s slot 的 0.1%。CPU 侧零瓶颈。**

---

## 13. 容量估算修正

研究阶段估算基于"zstd 压缩 3-5x（100KB→30KB）"。实测压缩率 25-33x（100KB→3-4KB），需修正：

### 13.1 修正后的带宽需求

| 场景 | 研究预估 | 实测修正 | 差异 |
|------|---------|---------|------|
| 10K × 100KB | 1 Gbps (未压缩) | **0.3 Gbps** (4KB 压缩) | 3.3x 低于预估 |
| 10K × 30KB (研究) | 300 Mbps | **0.3 Gbps** (4KB 压缩) | 基本吻合 |
| 50K (研究预估) | 1.5 Gbps (30KB) | **1.6 Gbps** (4KB) | 基本吻合 |
| 50K (实测压缩) | — | **1.6 Gbps** | 单 10G NIC 可承载 |
| 100K (极端) | 3 Gbps (30KB) | **3.1 Gbps** (4KB) | 双 10G NIC 或 25G NIC |

### 13.2 修正后的架构建议

```
原始研究估算:
  50K 手机 × 30KB 压缩 = 1.5 Gbps → 需 4 个 endpoint 分片 + 双万兆

修正后实际:
  50K 手机 × 4KB 压缩  = 1.6 Gbps → 单 10G NIC 即可
  100K 手机 × 4KB 压缩 = 3.1 Gbps → 双 10G NIC 或单 25G NIC
```

高压缩率使得：
- **单个 10G NIC 可支撑 50K 手机**（无需多 NIC bond）
- FEC 的紧迫性降低（带宽不再是首要瓶颈）
- 瓶颈从网络带宽转移至 **QUIC per-stream 开销**（瓶颈 2、3）

### 13.3 瓶颈迁移

```
优化前瓶颈分布:
  broadcast clone:  ████████████████████  (~1GB/block)    ← 已消除
  无压缩带宽:       ████████████████      (~1 Gbps/10K)   ← 已消除 (97%)
  QUIC stream/phone: ████████             (~10K open_uni)  ← 仍存在
  per-phone task:    ████                 (调度开销)        ← 缓解

优化后瓶颈分布 (Phase 1-3 完成):
  CPU 管线 (50K):   █                     8.4ms            ← 不是瓶颈
  网络传输 (50K):   ████                  ~30ms (@10G)     ← 可控
  QUIC stream:      █████████████         50K open_uni/块   ← 新首要瓶颈
  per-phone task:   ████████              50K tokio tasks   ← 次要瓶颈
```
...
| # | 工作项 | 原优先级 | 修正优先级 | 理由 |
|---|--------|---------|-----------|------|
| 1 | 流式分片发送（研究 Phase 2 残项） | 中 | **中** | 减少首字节延迟，但压缩后包很小 (4KB)，收益降低 |
| 2 | 完整出块→attestation 延迟链路 | — | **高** | register_block 传入出块时间，完善可观测性 |

### 中期（需条件成熟）

| # | 工作项 | 原优先级 | 修正优先级 | 理由 |
|---|--------|---------|-----------|------|
| 3 | QUIC Datagram + FEC | 高 | **中** | 带宽不再是首要瓶颈；但 Datagram 能消除 per-stream 开销 |
| 4 | 手机端负载均衡 | 中 | **中** | 需 app 更新配合 |

### 长期（需验证必要性）

| # | 工作项 | 原优先级 | 修正优先级 | 理由 |
|---|--------|---------|-----------|------|
| 5 | Pinned 线程 / io_uring | 中 | **低** | 压测显示 CPU 管线仅占 0.1% slot，tokio 调度不是瓶颈 |
| 6 | 预分配连接池 | 中 | **低** | HashMap lookup 已被 Arc 直接访问替代，5.1x 加速已足够 |
| 7 | SO_REUSEPORT 共享端口 | 低 | **低** | 多端口方案工作良好，客户端已适配 |

### 关键结论

1. **Phase 1-3 已将 CPU 侧开销从秒级降至毫秒级**，8.4ms/50K phones
2. **zstd 压缩率（97%）远超预期**，带宽需求从 39 Gbps 降至 1.6 Gbps
3. **当前架构的天花板在 QUIC per-stream 开销**，而非 CPU 或带宽
4. **Phase 3 深度优化的紧迫性显著降低**，50K 手机目标已可达成
5. **下一步最有价值的优化是 QUIC Datagram**（消除 per-stream 开销），而非 FEC（带宽已充裕）
