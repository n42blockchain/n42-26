...
- `cargo build --release -p n42-node-bin -p e2e-test`: passed.

### Ordered P0 Scenario 4

The first real Scenario 4 run used the independent S3 branch directly on the
still-unmodified `origin/main`. Its 1- and 3-node profiles passed, then the
5-node profile reproduced `[5, 5, 5, 0, 0]`: the two late joiners learned a
CommitQC but did not import the missing execution lineage. This is the exact
fresh-join regression found and fixed during S1 acceptance, not an S3 parent
selection regression.

Because the task book requires `S1 -> S2 -> S3`, the authoritative acceptance
run used a disposable integration worktree with the three independent commits
applied in that order. The combined tree passed `cargo check --all-targets`,
the S3 A/B regression, a fresh release build, and the complete full-profile
Scenario 4 command:
...
| Validator profile | Final heights | Result |
|---:|---|---|
| 1 | `[13]` | V1-V5 passed |
| 3 | `[13, 13, 13]` | V1-V5 passed |
| 5 | all five at 13 | V1-V5 passed |
| 7 | all seven at 14 | V1-V5 passed |
| 21 | all twenty-one at 16 | V1-V5 passed |

Every profile passed the sampled block-hash consistency check. The 21-node
profile averaged 8.0 seconds per block. The integration worktree lives under
the dedicated temporary worktree root and is not part of the S3 PR.
