...
## 补齐：移动端交叉编译、Xcode 项目、Android JNI (2026-02-17)

### 完成事项

#### 1. Scenario 8 Step 10: 验证节点收到 receipt

在 `mobile_bridge.rs` 中为 `ReceiptReceived` 事件添加 INFO 级别日志，在 scenario8 测试中读取节点日志验证收到的 receipt 数量与发送数一致。

测试结果：`receipt_log_count=2, receipts_sent=2` — 端到端 receipt 回传验证通过。

#### 2. iOS ARM64 交叉编译

**问题**：blst 库的 aarch64 汇编代码引用 `___chkstk_darwin`（macOS 栈探测函数），该符号在 iOS 上不存在。

**解决方案**：
- `BLST_PORTABLE=1` 环境变量无法解决（blst build.rs 仍使用汇编路径）
- cdylib 目标在 iOS 上不可用（会触发链接错误）
- 临时将 `crate-type` 改为 `["staticlib"]` 单独构建 iOS .a 文件

**构建命令**：
```bash
# Cargo.toml 中 crate-type 需临时改为 ["staticlib"]
BLST_PORTABLE=1 cargo build --release --target aarch64-apple-ios -p n42-mobile-ffi
```

**结果**：生成 40MB 的 `libn42_mobile_ffi.a`（ARM64 iOS 静态库），已复制到 `mobile/ios/N42Verifier/`。

#### 3. Xcode 项目

创建了 `mobile/ios/N42Verifier.xcodeproj/project.pbxproj`，配置：
- SwiftUI 应用（`N42VerifierApp.swift` 入口）
- Bridging Header 引用 `n42_mobile.h` C 头文件
- 链接 `libn42_mobile_ffi.a` 静态库
- 最低部署目标 iOS 16.0
- `LIBRARY_SEARCH_PATHS` 指向 `$(PROJECT_DIR)/N42Verifier`

#### 4. Android JNI 桥接

**设计决策**：Kotlin `N42Verifier.kt` 使用 JNI native methods，需要在 Rust 侧实现 JNI 函数映射。

新增 `crates/n42-mobile-ffi/src/android.rs`：
- 使用 `#[cfg(target_os = "android")]` 条件编译，不影响 iOS/主机构建
- 9 个 JNI 函数（`Java_com_n42_verifier_N42Verifier_native*`）
- 通过 `jni` crate (v0.21) 实现 Java ↔ Rust 类型转换
- 每个 JNI 函数委托给对应的 C FFI API 函数

**Android NDK 未安装**：当前环境没有 Android SDK/NDK，无法实际编译 .so 文件。已创建 `mobile/android/BUILD.md` 构建指南。

### 修改/新增文件

| 文件 | 变更 |
|------|------|
| `crates/n42-node/src/mobile_bridge.rs` | ReceiptReceived 添加 INFO 日志 |
| `tests/e2e/src/scenarios/scenario8_mobile_evm.rs` | 添加 step 10 节点 receipt 日志验证 |
| `crates/n42-mobile-ffi/Cargo.toml` | 添加 Android JNI 条件依赖 |
| `crates/n42-mobile-ffi/src/lib.rs` | 添加 `mod android` 条件导入 |
| `crates/n42-mobile-ffi/src/android.rs` | **新建**：JNI 桥接代码（9个函数） |
| `mobile/ios/N42Verifier.xcodeproj/project.pbxproj` | **新建**：Xcode 项目文件 |
| `mobile/ios/N42Verifier/libn42_mobile_ffi.a` | **新建**：iOS ARM64 静态库 |
| `mobile/ios/BUILD.md` | **新建**：iOS 构建指南 |
| `mobile/android/BUILD.md` | **新建**：Android 构建指南 |
| `mobile/android/app/src/main/jniLibs/arm64-v8a/.gitkeep` | **新建**：jniLibs 目录占位 |

### 已知限制

1. **blst iOS 交叉编译**：需要临时修改 `crate-type` 为 `["staticlib"]`，因为 cdylib 在 iOS 上链接 `___chkstk_darwin` 失败
2. **Android .so 未编译**：需要安装 Android NDK + cargo-ndk 后执行 `cargo ndk -t arm64-v8a build --release -p n42-mobile-ffi`
3. **iOS .a 文件较大**（40MB）：实际上线需要 strip + bitcode 优化

---

## 移动端代码：测试补全 + 安全审计 + 优化 (2026-02-17)

### 目标

基于已实现的移动 EVM 验证管线（Phase 1-6），进行三个维度的加固：
1. 修复 FFI 层安全审计发现的 7 类问题
2. 补全单元测试覆盖率（新增 17 个测试 + 2 个辅助测试）
3. 性能优化（bytecode 查找 O(N*M) → O(N)）

### 设计决策

**Phase 1 安全修复策略：**
- 采用集中式辅助函数方案：`lock_or_recover()` 和 `safe_cint()` 作为内部 helper，避免每处调用点重复写 poison 恢复和溢出检查逻辑
- Use-After-Free 修复选择显式 `abort()` recv_task 而非等待自然终止，因为 recv_task 持有 `Arc<Mutex<VecDeque>>` 引用，如果 VerifierContext 先被 drop，task 里的弱引用可能导致访问已释放内存
- QUIC 连接超时设为 10 秒——移动网络环境下过短会误判，过长影响用户体验

**Phase 2 测试策略：**
- verifier.rs 测试重点覆盖 `build_cached_reads` 私有函数的 5 种分支（空包、EOA、cache 命中、uncached 命中、缺失）和 `verify_block` 的 3 种错误路径
- FFI 测试侧重内部数据结构的正确性（VerifyStats、JSON 序列化格式），不测试 QUIC 网络逻辑
- Cargo.toml 添加 `"lib"` crate-type 使 `cargo test` 可用（staticlib/cdylib 不支持 test harness）

**Phase 3 优化选择：**
- 仅做 bytecode 查找优化，`Bytes::clone()` 已是 Arc 引用计数 O(1)，`recv_loop` 内存分配收益极小

### 实施细节

#### Phase 1: FFI 安全修复（7 项）

| # | 严重度 | 修复内容 | 文件 |
|---|--------|----------|------|
| 1.1 | CRITICAL | 6 个 FFI 函数添加 NULL 指针检查（host, out_buf, data） | `n42-mobile-ffi/src/lib.rs` |
| 1.2 | HIGH | `n42_verifier_free()` 显式 abort recv_task 防止 Use-After-Free | 同上 |
| 1.3 | HIGH | `safe_cint()` 辅助函数替换 3 处 `as c_int` 防溢出 | 同上 |
| 1.4 | HIGH | `lock_or_recover()` 替换 12 处 `.lock().unwrap()` 防 mutex poison 级联 panic | 同上 |
| 1.5 | MEDIUM | `connect_quic()` 添加 10 秒 `tokio::time::timeout` | 同上 |
| 1.6 | MEDIUM | `recv_loop` 队列满时添加 `warn!` 日志 | 同上 |
| 1.7 | LOW | `_slot_key` → `slot_key`、`SkipServerVerification` 安全警告文档、`state_root_match` 注释 | `mobile_packet.rs` + `lib.rs` |

#### Phase 2: 测试补全（19 个新测试）

| 模块 | 新增测试数 | 总测试数 | 覆盖点 |
|------|-----------|----------|--------|
| `n42-mobile/verifier.rs` | 10 | 10 | build_cached_reads 5 分支 + verify_block 3 错误路径 + update_cache |
| `n42-node/packet_builder.rs` | 2 | 5 | account+storage 完整解析、preimage 缺失错误 |
| `n42-node/mobile_bridge.rs` | 2 | 2 | receipt 聚合逻辑、4 种 HubEvent 不 panic |
| `n42-mobile-ffi/lib.rs` | 5 | 5 | VerifyStats 默认值、JSON 序列化、safe_cint 正常/溢出 |

#### Phase 3: 性能优化

`build_cached_reads()` 中 bytecode 查找从 `Vec::find()` 改为预构建 `HashMap<&B256, &Bytes>`：
```rust
let uncached_map: HashMap<&B256, &Bytes> = packet
    .uncached_bytecodes.iter().map(|(h, b)| (h, b)).collect();
// O(1) lookup: uncached_map.get(&wa.code_hash)
```

### 阶段完成状态

- [x] Phase 1: FFI 安全审计修复（7/7 项）
- [x] Phase 2: 单元测试补全（19 个新测试全部通过）
- [x] Phase 3: bytecode 查找优化
- [x] 全量回归：45 个测试通过（n42-mobile 33 + n42-node 7 + n42-mobile-ffi 5）

### 修改文件清单

| 文件 | 变更 |
|------|------|
| `crates/n42-mobile-ffi/src/lib.rs` | NULL 检查、mutex recovery、safe_cint、UAF 修复、QUIC 超时、队列警告、文档、测试 |
| `crates/n42-mobile-ffi/Cargo.toml` | crate-type 添加 `"lib"` 支持 cargo test |
| `crates/n42-mobile/src/verifier.rs` | HashMap 优化 + 10 个单元测试 |
| `crates/n42-node/src/packet_builder.rs` | 2 个新测试 |
| `crates/n42-node/src/mobile_bridge.rs` | `process_receipt` 改为 pub + 2 个新测试 |
| `crates/n42-node/src/mobile_packet.rs` | `_slot_key` → `slot_key` 变量名修复 |
...
对 8 个子系统进行审计后，修复剩余缺口：
- **聚合结果回馈**：MobileVerificationBridge 发送 AttestationEvent，main.rs 接线
- **CacheSync 接线**：FFI recv_loop 处理 0x02 CacheSyncMessage，更新本地 CodeCache
- **State Sync 超时**：orchestrator.rs 添加 30s 超时保护（已在前一阶段完成）

### 1. 聚合结果回馈到 Orchestrator

**设计决策**：AttestationEvent 通过 mpsc channel 从 MobileVerificationBridge 传递。当前阶段只做日志输出，为后续 consensus 策略调整预留接口。

**实现**：
- `mobile_bridge.rs`：已在前阶段重写，包含 `AttestationEvent` 结构体和 `with_attestation_tx()` builder
- `main.rs`：创建 `mpsc::unbounded_channel()`，传入 bridge，spawn 日志任务消费事件
...
// spawn attestation logger task
```

### 2. CacheSync 接线（FFI 层）

**设计决策**：`VerifierContext.code_cache` 从 `Mutex<CodeCache>` 改为 `Arc<Mutex<CodeCache>>`，以便 recv_loop 异步任务共享访问。这是最小化改动——所有现有 `lock_or_recover(&ctx.code_cache)` 调用不变（`Arc` 自动 deref）。

**实现细节**：
- `code_cache.rs`：新增 `remove()` 方法支持 evict_hints
- `lib.rs`：
  - `code_cache` 字段类型改为 `Arc<Mutex<CodeCache>>`
  - `n42_connect()` 传递 `code_cache.clone()` 给 recv_loop
  - `recv_loop()` 新增 `code_cache` 参数，0x02 消息反序列化为 `CacheSyncMessage`，执行 insert/remove 操作
...
0x02 => {
    match bincode::deserialize::<CacheSyncMessage>(payload) {
        Ok(msg) => {
            let mut cache = lock_or_recover(&code_cache);
            for (hash, code) in msg.codes { cache.insert(hash, code); }
            for hash in &msg.evict_hints { cache.remove(hash); }
        }
        Err(e) => warn!("failed to decode CacheSyncMessage: {}", e),
    }
}
```

### 3. State Sync 超时（已完成确认）

- `SYNC_REQUEST_TIMEOUT = 30s`
- `sync_started_at: Option<Instant>` 字段跟踪请求时间
- `initiate_sync()` 检测超时后自动重置 `sync_in_flight`

### 验证结果

- `cargo check -p n42-mobile-ffi` ✅
- `cargo check -p n42-node-bin` ✅
- `cargo test -p n42-mobile -p n42-mobile-ffi -p n42-node` ✅ (36 tests passed)

### 修改文件清单

| 文件 | 变更 |
|------|------|
| `bin/n42-node/src/main.rs` | 添加 attestation channel 接线 + logger task |
| `crates/n42-mobile-ffi/src/lib.rs` | `code_cache` → `Arc<Mutex<CodeCache>>`，recv_loop 处理 CacheSyncMessage |
| `crates/n42-mobile/src/code_cache.rs` | 新增 `remove()` 方法 |

### 后续计划

- QC-in-extra-data：alloy Engine API 限制（32 字节 extra_data），需要自定义 Engine API 类型才能实现，标记为架构决策
- 多 peer sync 重试：当前选第一个连接 peer，peer 断开后自动选下一个，暂时够用
- Attestation 事件消费：后续可接入 consensus 策略（如降低 attestation 不足区块的优先级）

---

## 第二轮测试补全 + 安全审计 + 优化（2026-02-17）

### 设计决策

1. **`register_block()` 幂等化**：发现 `process_receipt()` 每次调用都先 `register_block()`，导致已有 receipt 计数被清零。根本原因是 `HashMap::insert()` 无条件覆盖。修复为 `contains_key` 检查后提前返回，保留已有的 `BlockVerificationStatus`。

2. **差异化传输**：CodeCache 框架虽已搭建但实际未减少带宽——`packet_builder` 直接包含 `witness.codes` 全量。改为在 `mobile_packet_loop` 中维护 `previously_sent_codes: HashMap<B256, Bytes>`，每个区块只传输新增的 bytecodes。新手机连接时通过 `broadcast_cache_sync()` 发送全量缓存（广播模式，后续可优化为 per-session 定向发送）。

3. **丢包计数器**：FFI recv_loop 队列满时静默丢包，添加 `Arc<AtomicU64>` 计数器，暴露在 `n42_get_stats` JSON 输出中。

4. **apply_cache_sync 提取**：将 recv_loop 中的 CacheSyncMessage 处理逻辑提取为独立函数 `apply_cache_sync()`，使其可在不启动 QUIC 连接的情况下进行单元测试。

### 安全修复清单

| ID | 严重性 | 问题 | 修复 |
|----|--------|------|------|
| 1.1 | CRITICAL | `register_block()` 无条件覆盖导致 receipt 计数清零 | 添加 `contains_key` 幂等检查 |
| 1.3 | MEDIUM | recv_loop 丢包无追踪 | `dropped_count: Arc<AtomicU64>` + stats 输出 |
| 1.4 | LOW | threshold=0 语义异常 | `debug_assert!(threshold >= 1)` |
| 1.5 | CRITICAL | CodeCache 不减少任何传输带宽 | `previously_sent_codes` 差异过滤 + PhoneConnected 触发 CacheSyncMessage |

### 差异化传输数据流

```
Block N:  witness.codes = [USDC, UniV3, NewContract]
          previously_sent = {}
          → packet.uncached_bytecodes = [USDC, UniV3, NewContract]  (全量)
          → previously_sent = {USDC, UniV3, NewContract}

Block N+1: witness.codes = [USDC, UniV3, AnotherNew]
           previously_sent = {USDC, UniV3, NewContract}
           → packet.uncached_bytecodes = [AnotherNew]  (仅增量！)
           → previously_sent = {USDC, UniV3, NewContract, AnotherNew}

PhoneConnected:
           → broadcast_cache_sync({USDC, UniV3, NewContract, AnotherNew})
```

### 实施细节

- `previously_sent_codes` 容量上限 2000 条（约 100 个区块的 codes），超出后随机淘汰
- `MobileVerificationBridge` 新增 `phone_connected_tx` 通道，在 `PhoneConnected` 事件时通知 `mobile_packet_loop`
- `mobile_packet_loop` 使用 `tokio::select!` 同时监听区块提交和手机连接两个通道

### 新增测试（11 个）

| 文件 | 测试 | 验证点 |
|------|------|--------|
| code_cache.rs | `test_code_cache_remove` | remove 已有/不存在 key + len 变化 |
| code_cache.rs | `test_code_cache_clear` | clear 后 len=0 + is_empty |
| code_cache.rs | `test_cache_sync_message_roundtrip` | bincode 序列化/反序列化一致 |
| code_cache.rs | `test_cache_sync_message_empty` | 空消息正常序列化 |
| verification.rs | `test_register_block_no_overwrite` | 重复注册不覆盖 receipt 计数 |
| verification.rs | `test_threshold_one_immediate_attestation` | threshold=1 时首个 receipt 触发 attestation |
| mobile_bridge.rs | `test_bridge_multiple_blocks` | 不同区块的 receipt 分别聚合 |
| mobile_bridge.rs | `test_bridge_duplicate_receipt_ignored` | 同 verifier 重复 receipt 被忽略 |
| mobile_bridge.rs | `test_bridge_no_attestation_tx` | 不设 attestation_tx 达阈值不 panic |
| lib.rs (FFI) | `test_apply_cache_sync` | CacheSyncMessage 正确更新 CodeCache |
| orchestrator.rs | `test_sync_timeout_resets_in_flight` | sync 超时后重置并允许新请求 |

另外重写了 `test_bridge_processes_receipts`，不再绕过 `bridge.process_receipt()`。

### 验证结果

- `cargo check -p n42-mobile -p n42-mobile-ffi -p n42-node -p n42-node-bin` ✅
- `cargo test -p n42-mobile` ✅ (39 tests passed, 原 33 + 新 6)
- `cargo test -p n42-mobile-ffi` ✅ (6 tests passed, 原 5 + 新 1)
- `cargo test -p n42-node` ✅ (40 tests passed, 原 36 + 新 4)

### 修改文件清单

| 文件 | 变更 |
|------|------|
| `crates/n42-mobile/src/verification.rs` | `register_block()` 幂等化 + threshold 断言 + 2 新测试 |
| `crates/n42-mobile/src/code_cache.rs` | 4 新测试 |
| `crates/n42-node/src/mobile_bridge.rs` | `phone_connected_tx` 通道 + 重写 1 测试 + 3 新测试 |
| `crates/n42-mobile-ffi/src/lib.rs` | 丢包计数器 + `apply_cache_sync()` 提取 + 1 新测试 |
| `crates/n42-node/src/orchestrator.rs` | 1 新测试 |
| `crates/n42-node/src/mobile_packet.rs` | 差异化传输 + PhoneConnected 触发 CacheSyncMessage |
| `bin/n42-node/src/main.rs` | `phone_connected` 通道接线 |

### 后续计划

- Per-session 定向发送：当前 PhoneConnected 时广播全量 codes 给所有手机（冗余但无害），后续可实现 per-session 发送仅推送给新手机
- `previously_sent_codes` 淘汰策略改进：当前使用 HashMap 随机淘汰，可改为 LRU 或按 block_number 有序淘汰
- CacheInventoryReceived 事件利用：手机上报的 cache 清单可用于更精准的 per-session 差异传输

---

## Phase 5: 全链路生产就绪度补齐（2026-02-17）

### 背景

通过三个并行审计 agent 和两个验证 agent 对整体系统进行全面生产就绪度审计，共发现约 50 项问题，交叉验证后筛选出 17 项真实缺陷，按安全影响分级实施修复。

### 设计决策

**bincode size limit 实现方式**：最初使用 `bincode::options().with_limit()` API，但发现它与 `bincode::serialize()` 的默认编码格式（varint vs fixint）不兼容，会导致反序列化失败。最终改为在反序列化前先检查输入字节长度——由于数据已在内存中（来自 GossipSub/QUIC stream），长度检查足以防止 OOM，同时保持与标准 `bincode::serialize` 的完全兼容。

**Equivocation 检测架构**：VoteCollector 按 (view, block_hash) 创建，无法直接看到跨 block_hash 的投票。因此在 ConsensusEngine 层添加了 `equivocation_tracker: HashMap<u32, B256>`，在 `process_vote` 中 block_hash 匹配检查之前记录每个 validator 的投票，检测到同 view 不同 hash 的矛盾投票时发出 `EquivocationDetected` 事件。

**动态 Attestation Threshold**：`max(10, connected_count * 2/3)` 公式保证了两个属性：(1) 初始部署手机少时下限 10 提供基本保护，(2) 大规模部署时 2/3 阈值与 BFT 理论一致。Bridge 内部维护 connected_count 计数器而非查询 StarHub，避免跨组件耦合。

**Epoch 持久化**：在 `ConsensusSnapshot` 中新增 `scheduled_epoch_transition` 字段，使用 `#[serde(default)]` 确保向后兼容旧 snapshot 文件。EpochManager 新增 `staged_info` 字段保存原始 `ValidatorInfo`，避免从 ValidatorSet 反向提取。

### 实施细节

#### Phase 1: CRITICAL 安全修复

| ID | 修复 | 文件 |
|----|------|------|
| C1 | `process_receipt()` 添加 `verify_signature()` 调用，无效签名直接丢弃 | `mobile_bridge.rs` |
| C2 | 所有 `bincode::deserialize` 添加输入长度检查（receipt 64KB, consensus msg 4MB, CacheSync 16MB） | `star_hub.rs`, `handlers.rs`, `lib.rs` |
| C3 | `SkipServerVerification` 替换为 `PinnedCertVerification`（SHA-256 证书固定）；证书持久化到 `cert_dir` | `star_hub.rs`, `lib.rs` |
| C4 | receipt stream 读取添加 10 秒 `tokio::time::timeout` | `star_hub.rs` |

#### Phase 2: HIGH 可靠性修复

| ID | 修复 | 文件 |
|----|------|------|
| H1 | `ViewChanged` 时 `pending_block_data.clear()` 改为条件清空（仅在无 pending_finalization 时） | `orchestrator.rs` |
| H2 | per-phone 最小 2 秒间隔限速 | `star_hub.rs` |
| H3 | broadcast channel 容量从硬编码 256 改为 `StarHubConfig.broadcast_buffer_size` | `star_hub.rs` |
| H4 | orchestrator `run()` 循环退出后执行最终 state persistence | `orchestrator.rs` |
| 新增 | 动态 attestation threshold: `max(10, connected_count * 2/3)` | `mobile_bridge.rs` |

#### Phase 3: 共识健壮性

| ID | 修复 | 文件 |
|----|------|------|
| H5 | `ConsensusEngine` 新增 `equivocation_tracker`，检测同 view 矛盾投票并发出 `EquivocationDetected` 事件 | `state_machine.rs`, `orchestrator.rs` |
| H6 | `ConsensusSnapshot` 新增 `scheduled_epoch_transition` 字段；recovery 时恢复 staged epoch | `persistence.rs`, `epoch.rs`, `main.rs` |

### 修改文件清单

| 文件 | 变更 |
|------|------|
| `Cargo.toml` (workspace) | 添加 `sha2 = "0.10"` |
| `crates/n42-network/Cargo.toml` | 添加 `sha2`, `hex` |
| `crates/n42-mobile-ffi/Cargo.toml` | 添加 `sha2` |
| `crates/n42-network/src/gossipsub/handlers.rs` | 4MB size limit |
| `crates/n42-network/src/mobile/star_hub.rs` | receipt size limit + stream timeout + rate limit + broadcast buffer config + cert pinning |
| `crates/n42-mobile-ffi/src/lib.rs` | CacheSync size limit + PinnedCertVerification |
| `crates/n42-node/src/mobile_bridge.rs` | verify_signature + dynamic threshold + connected_count |
| `crates/n42-node/src/orchestrator.rs` | conditional clear + graceful shutdown + epoch snapshot + equivocation handling |
| `crates/n42-node/src/persistence.rs` | scheduled_epoch_transition 字段 |
| `crates/n42-consensus/src/protocol/state_machine.rs` | equivocation_tracker + EquivocationDetected + locked_qc/last_committed_qc/consecutive_timeouts 访问器 |
| `crates/n42-consensus/src/validator/epoch.rs` | staged_info + staged_epoch_info() |
| `bin/n42-node/src/main.rs` | epoch recovery on startup |

### 验证结果

- 全量编译通过（包括 n42-node-bin）
- 258 个测试全部通过，0 failures
- 性能 bench 无回归（500 validators 场景正常）

### 后续计划

- View Synchronization 协议（需要新消息类型）
- Prometheus 指标集成（独立功能）
- 配置文件支持（TOML，运维工具链）
- GossipSub scoring 调优（需真实多节点环境测试）
- M1 production chainspec、M6 witness 完整性检查（可选项）

---

## 第二轮生产就绪度审计与修复 (2026-02-17)

### 审计方法

逐行阅读 state_machine.rs、orchestrator.rs、star_hub.rs、mobile_packet.rs、quorum.rs、persistence.rs 的完整代码，交叉验证消息签名格式一致性。

### 设计决策

1. **独立 `verify_commit_qc()` 函数**：新建独立函数而非给 `verify_qc` 加参数。PrepareQC 使用 `view || block_hash`，CommitQC 使用 `"commit" || view || block_hash`，调用者应明确选择正确的验证函数。与 `verify_tc()` 保持一致的 API 风格，避免在所有现有调用点传递额外参数。

2. **Snapshot 使用 engine 实际值**：`locked_qc` 在 engine 内部可能高于当前 `commit_qc`（例如通过 piggybacked QC 或 NewView 更新），持久化 `commit_qc` 作为 `locked_qc` 会导致 crash 恢复后锁降级。

3. **确定性 peer 选择**：用 `local_view % len` 而非引入 rand 依赖。不同 view 自然选择不同 peer；连续 retry 同一 view 会选同一 peer，但 timeout 后 view 会变，自动切换。

4. **CodeCache FIFO 淘汰**：用 `VecDeque<B256>` + `HashMap<B256, Bytes>` 实现插入顺序淘汰。比 LRU crate 更轻量，且对 code cache 场景（重复命中率高、淘汰最老即可）已足够。

5. **Session 原子计数器**：将 `packets_sent`/`receipts_received` 改为 `AtomicU64`，`last_active` 改为原子存储偏移量。receipt/broadcast 路径从 write lock 降级为 read lock，允许 2500+ 手机并发读。
...
| ID | 问题 | 修复 |
|----|------|------|
| S1 | `process_decide()` 不验证 CommitQC BLS 聚合签名 | 新增 `verify_commit_qc()` 并在 `process_decide()` 中调用 |
| S2 | `verify_qc()` 用 prepare 格式验证 CommitQC，sync 验证永远失败 | orchestrator `handle_sync_response` 改用 `verify_commit_qc()` |
| S3 | Snapshot 用 `commit_qc` 代替 `engine.locked_qc()` | 改用 `self.engine.locked_qc().clone()` |

#### HIGH — 可靠性问题

| ID | 问题 | 修复 |
|----|------|------|
| R1 | Payload build spawned task 无超时 | 添加 `tokio::time::timeout(30s)` |
| R2 | Sync peer 始终选 `iter().next()` | 改用 `local_view % peers.len()` 确定性轮换 |
| R3 | view 作为 block_number 传递 | 标记为已知限制，当前 view == block_number，后续处理 |

#### MEDIUM — 生产增强

| ID | 问题 | 修复 |
|----|------|------|
| P1 | `previously_sent_codes` HashMap 随机淘汰 | 替换为 `CodeCache`（VecDeque + HashMap），FIFO 淘汰 |
| P2 | Session HashMap 每次取 write lock | 计数器改 AtomicU64，receipt/broadcast 用 read lock |

### 修改文件清单

| 文件 | 变更 |
|------|------|
| `crates/n42-consensus/src/protocol/quorum.rs` | 新增 `verify_commit_qc()` + 4 个测试 |
| `crates/n42-consensus/src/lib.rs` | re-export `verify_commit_qc` |
| `crates/n42-consensus/src/protocol/state_machine.rs` | `process_decide()` 添加 CommitQC 签名验证 + 1 个测试 |
| `crates/n42-node/src/orchestrator.rs` | sync 验证修复 + snapshot 修复 + payload 超时 + peer 轮换 |
| `crates/n42-node/src/mobile_packet.rs` | `CodeCache` 替换 HashMap 随机淘汰 |
| `crates/n42-network/src/mobile/session.rs` | `AtomicU64` 计数器 + 原子 `last_active` |
| `crates/n42-network/src/mobile/star_hub.rs` | write lock → read lock |

### 验证结果

- n42-consensus: 80 个单元测试通过（含 5 个新测试），67 个集成测试通过
- n42-node: 40 个测试通过
- n42-network/n42-mobile/n42-mobile-ffi: 编译通过
- n42-node-bin: 编译通过
- 总计：187+ 测试全部通过
...
| 测试 | 验证内容 |
|------|---------|
| `test_verify_commit_qc_valid` | CommitQC 使用正确签名格式可通过验证 |
| `test_verify_commit_qc_insufficient_signers` | 不足 quorum 的 CommitQC 被拒绝 |
| `test_verify_qc_rejects_commit_qc` | verify_qc 不接受 CommitQC（签名格式不同） |
| `test_verify_commit_qc_rejects_prepare_qc` | verify_commit_qc 不接受 PrepareQC |
| `test_decide_rejects_forged_signature` | 伪造聚合签名的 Decide 消息被拒绝 |
...
- Prometheus 指标集成
- 配置文件支持（TOML）
- GossipSub scoring 调优
- R3: block_number 与 view 分离（待生产 testnet 验证）

---

## Blockscout 区块浏览器集成 (2026-02-17)

### 设计决策

1. **RPC 完备度评估**：经过对 reth v1.11.0 RPC 模块的全面审计，确认 reth 已 100% 覆盖 Blockscout 所需的所有 RPC 接口（eth/debug/trace/net/web3/txpool），无需实现任何新的 RPC 方法。
2. **默认值覆盖方案**：使用 reth 提供的 `DefaultRpcServerArgs::try_init()` 机制在 `Cli::parse()` 之前设置自定义默认值。选择此方案而非硬编码 CLI 参数，原因是：用户仍可通过 `--http.api`、`--ws.api` 等 CLI 参数覆盖默认值，保持灵活性。
3. **JSONRPC_VARIANT 选择 geth**：Blockscout 支持 geth/nethermind/erigon 等变体。reth 的 RPC 接口与 geth 兼容（`debug_traceTransaction` 支持 `callTracer`），因此配置为 `geth` 变体。
4. **WebSocket 默认启用**：Blockscout 需要 `eth_subscribe(newHeads)` 来实时追踪新区块，因此 WS 必须默认开启。

### 实施细节

#### Phase 1: RPC 默认配置 (`bin/n42-node/src/main.rs`)

在 `fn main()` 入口、`Cli::parse()` 之前调用 `DefaultRpcServerArgs::try_init()`：

- **HTTP 默认启用**，模块：`eth,net,web3,debug,trace,txpool,rpc,n42`
- **WebSocket 默认启用**，模块：`eth,net,web3`（仅订阅所需）
- **CORS 默认 `*`**：允许 Blockscout 前端跨域访问

新增依赖：`reth-node-core`（`DefaultRpcServerArgs`）、`reth-rpc-server-types`（`RethRpcModule`、`RpcModuleSelection`）

#### Phase 2: Testnet 脚本更新 (`scripts/local-testnet.sh`)

- `--http.api` 新增 `debug,trace,txpool`
- 新增 `--http.corsdomain "*"`
- 新增 `--ws`、`--ws.port`、`--ws.api` 参数

#### Phase 3: Blockscout Docker Compose (`docker/`)

- `docker-compose.blockscout.yml`：包含 PostgreSQL、Redis、Blockscout 后端/前端、Smart Contract Verifier
- `blockscout.env`：独立环境变量文件，关键配置包括 `ETHEREUM_JSONRPC_VARIANT=geth`、`INDEXER_INTERNAL_TRANSACTIONS_TRACER_TYPE=callTracer`、`CHAIN_ID=42`

#### Phase 4: RPC 验证脚本 (`scripts/verify-blockscout-rpc.sh`)

轻量级 bash 脚本，检查 Blockscout 必需的所有 RPC 方法是否可用，包括 eth_*/debug_*/net_*/web3_*/trace_*/txpool_* 以及 WebSocket 连接。

### 修改文件清单

| 文件 | 操作 | 说明 |
|------|------|------|
| `Cargo.toml` (workspace) | 修改 | 新增 `reth-node-core`、`reth-rpc-server-types` 依赖 |
| `bin/n42-node/Cargo.toml` | 修改 | 新增 `reth-node-core`、`reth-rpc-server-types` 依赖 |
| `bin/n42-node/src/main.rs` | 修改 | 添加 `DefaultRpcServerArgs::try_init()` 设置 Blockscout 所需的 RPC 默认值 |
| `scripts/local-testnet.sh` | 修改 | 更新 RPC 参数，启用 WS |
| `docker/docker-compose.blockscout.yml` | 新建 | Blockscout 完整部署配置 |
| `docker/blockscout.env` | 新建 | Blockscout 环境变量 |
| `scripts/verify-blockscout-rpc.sh` | 新建 | RPC 端点验证脚本 |

### 验证状态

- [x] `cargo check -p n42-node-bin` 编译通过
- [x] 无新增 warning
- [ ] 节点启动验证（需手动启动）
- [ ] Blockscout 索引验证（需 Docker 环境）

### 后续计划

- 节点启动后运行 `scripts/verify-blockscout-rpc.sh` 验证全部 RPC 方法
- 使用 `docker compose -f docker/docker-compose.blockscout.yml up -d` 启动 Blockscout 并验证索引
- 考虑添加 Prometheus 指标导出供 Blockscout Stats 使用

---

## 生产就绪度加固 — 安全性、可靠性、共识健壮性 (2026-02-17)

### 背景

在 Blockscout 集成完成后，对 n42-consensus、n42-network、n42-node 三个核心 crate 进行全面审计，识别出 CRITICAL/HIGH/MEDIUM 级别问题共计 50+。本次集中修复最高优先级的安全和可靠性问题。

### 设计决策

1. **bitmap 验证优先于签名验证**：在 `collect_signer_keys()` 入口处校验 bitmap 长度，而非依赖后续的 `get_public_key()` 边界检查。这样在验证签名之前就能拒绝格式错误的证书，是更强的防御层。

2. **unwrap() 统一替换策略**：共识引擎中的 6 个 `unwrap()` 都在 `Option` 类型上，采用 `match + warn + return Ok(())` 模式而非 `expect()`，因为共识引擎不应因 collector 未初始化而 panic。在分布式系统中，节点 panic = 减少活跃验证者数 = 影响网络安全性。

3. **BTreeMap 替代 HashMap**：`pending_block_data` cache 改用 BTreeMap，利用其有序性使 FIFO 驱逐策略变为确定性的。B256 hash 的 Ord 实现保证了一致的排序。对于仅 16 个条目的 cache，BTreeMap vs HashMap 性能差异可忽略。

4. **TOCTOU 修复方案**：没有使用 AtomicU64 计数器（会增加复杂度），而是在 session 插入前通过 write lock 进行二次检查。虽然会短暂增加锁争用，但连接建立不在热路径上（~1 conn/sec vs ~10,000 packets/sec），而且确保了计数的精确性。

5. **fsync 策略**：在 `persistence.rs` 的 rename 前添加 `sync_all()`。虽然增加了延迟（~1-2ms on SSD），但共识状态持久化每 8 秒最多一次，不在性能关键路径上。
...
| # | 模块 | 文件 | 问题 | 修复 |
|---|------|------|------|------|
| C-1 | consensus | `quorum.rs:332` | signers bitmap 长度未验证，短 bitmap 可绕过法定人数检查 | 添加 `signers.len() != validator_set.len()` 检查 |
| C-2 | consensus | `state_machine.rs` | 6 处 `unwrap()` 在 Option 上（line 572, 608, 628, 720, 758, 929） | 替换为 match + warn + early return |
| C-3 | node | `orchestrator.rs:516` | `now_ms / slot_ms` 当 slot_ms=0 时除零 panic | 添加零值守卫，fallback 到 1 秒 |
| C-4 | node | `orchestrator.rs:627` | payload 序列化失败时 `unwrap_or_default()` 默默产生空 vec | 改为 match + error log + return |
| C-5 | node | `orchestrator.rs:1136` | sync 响应无块数上限，恶意请求可触发 OOM | 添加 `MAX_SYNC_BLOCKS = 128` 限制 |
| C-6 | network | `star_hub.rs:576` | `idle_timeout_secs` 配置字段被忽略，硬编码 300s | 传递配置值到 `build_server_config()` |
| C-7 | network | `star_hub.rs:249` | 连接限制检查存在 TOCTOU 竞态 | 在 write lock 下二次检查 |

#### HIGH 级别

| # | 模块 | 文件 | 问题 | 修复 |
|---|------|------|------|------|
| H-1 | network | `star_hub.rs:543` | 私钥文件无权限限制 | 添加 `chmod 0o600`（仅 Unix） |
| H-2 | network | `star_hub.rs:576` | `IdleTimeout::try_from().unwrap()` 可能 panic | 改为 `if let Ok(...)` |
| H-3 | network | `service.rs:159` | `pending_sync_channels` 无界增长 | 添加 64 条上限 + stale 驱逐 |
| H-4 | node | `orchestrator.rs:914` | `pending_block_data` 非确定性 HashMap 驱逐 | 改用 BTreeMap 确保确定性 |
| H-5 | node | `tx_bridge.rs:152` | `VecDeque::contains()` O(n) 线性扫描 10,000 条目 | 拆分为 VecDeque + HashSet，O(1) 查询 |
| H-6 | node | `persistence.rs:46` | rename 前无 fsync，崩溃可能丢数据 | 添加 `file.sync_all()` |
| H-7 | node | `mobile_bridge.rs:98` | `connected_count` 简单加减可因重复事件漂移 | 改用 `HashSet<u64>` 精确跟踪 |

### 关键代码变更

#### 1. quorum.rs — bitmap 长度验证

```rust
// 在 collect_signer_keys() 入口处
let expected_len = validator_set.len() as usize;
if signers.len() != expected_len {
    return Err(ConsensusError::InvalidQC/TC { reason: "bitmap length mismatch" });
}
```

#### 2. state_machine.rs — unwrap 消除示例

```rust
// Before: self.vote_collector.as_mut().unwrap()
// After:
let collector = match self.vote_collector.as_mut() {
    Some(c) => c,
    None => {
        tracing::warn!(view, "vote_collector not initialized");
        return Ok(());
    }
};
```

#### 3. orchestrator.rs — 除零守卫
...
#### 4. tx_bridge.rs — O(1) dedup

```rust
// VecDeque 维护插入顺序做 FIFO 驱逐，HashSet 提供 O(1) 查询
...
- `cargo check` — 通过（唯一 warning: `load_validator_set` 未使用，为已有问题）
- `cargo test -p n42-consensus -p n42-node -p n42-network` — **40 测试全部通过**，无回归

### 后续计划

- 考虑添加 `service.rs` 中 `InboundFailure` 事件的 request_id 追踪，实现更精确的 channel 清理
- `state_machine.rs` 中被跳过的 `unwrap_or_default()` 模式（line 748-752）虽已修复为单次 match，但 `try_form_commit_qc` 与 `try_form_prepare_qc` 的对称结构可进一步统一
- 考虑将 `pending_block_data` BTreeMap 改为带 TTL 的 LRU cache
- 审计 `epoch.rs:109` 的 `unreachable!()` 宏，确认是否可达

---

## 第三轮生产就绪度补齐（2026-02-17）

### 概览

基于全面审计发现的 8 项 BLOCKING + 16 项 IMPORTANT 问题，分 6 个 Phase 完成补齐。共修改约 20 个文件，新增 1 个模块（keystore），所有 258 个测试通过。

### Phase 1：配置基础设施 + Chain ID 修复（B1, B7）

**设计决策**：
- `ConsensusConfig::from_file()` 支持 TOML 和 JSON 格式，通过文件扩展名区分
- 优先从 `N42_CONSENSUS_CONFIG` 环境变量加载配置文件，回退到 dev 模式保持向后兼容
- Chain ID 统一为 4242（代码中的值），修复 Blockscout 配置不匹配

**修改文件**：`Cargo.toml`(workspace), `n42-chainspec/Cargo.toml`, `n42-chainspec/src/lib.rs`, `main.rs`, `blockscout.env`, `docker-compose.blockscout.yml`

### Phase 2：消息版本控制 + 未来 view 缓冲（B6, I1）

**设计决策**：
- `VersionedMessage` 包装 `version: u16` + `ConsensusMessage`，编解码端检查版本号
- 解码时先尝试 VersionedMessage，失败后回退到裸 ConsensusMessage（向后兼容升级过渡期）
- 未来 view 缓冲：FUTURE_VIEW_WINDOW=5, MAX_FUTURE_MESSAGES=64
- `Decide`/`NewView` 消息豁免缓冲（它们本身就是跨 view 的消息）
- 两阶段 drain 避免借用冲突：先收集到 Vec，再分类处理

**修改文件**：`messages.rs`, `consensus/mod.rs`, `lib.rs`(primitives+consensus), `handlers.rs`, `state_machine.rs`

### Phase 3：State Sync 修复（B3, B4, I11）

**设计决策**：
- 同步缓冲区从 1000 扩大到 10000（约 22 小时，可通过 `N42_SYNC_BUFFER_SIZE` 配置）
- Leader payload 回传：spawned build task 通过 `leader_payload_tx` 通道将已编码的块数据发回，orchestrator 在 select! 新分支中消费并填充 committed_blocks
- Sync QC 验证：当 `validator_set_for_sync` 为 None 时拒绝而非跳过

**修改文件**：`orchestrator.rs`

### Phase 4：Epoch 转换集成（B2, I15）

**设计决策**：
- `EngineOutput::EpochTransition { new_epoch, validator_count }` 新变体
- epoch boundary 检查在 `advance_to_view()` 中，view 推进之前触发
- orchestrator 处理 EpochTransition 时更新 `validator_set_for_sync`，确保新 epoch 的同步块可被验证
- `main.rs` 三个初始化路径（snapshot/fresh/error）均支持 `epoch_length`

**修改文件**：`state_machine.rs`, `orchestrator.rs`, `main.rs`

### Phase 5：通道反压 + QC 修复 + 健康检查（I5, I6, I8）

**设计决策**：
- 6 个 `unbounded_channel` 改为有界 channel（容量：output 1024, attest 256, mobile/phone 128, tx 4096）
- ConsensusEngine 的 `emit()` 使用 `try_send()` （同步上下文），其他发送端同理
- 内部通道（block_ready, leader_payload, net_event, hub_event）保持无界——它们是模块内部的短生命周期通道
- Adapter QC 验证：先尝试 PrepareQC 格式，再尝试 CommitQC 格式（`verify_qc().or_else(verify_commit_qc)`）
- 新增 `n42_health` RPC 方法，返回 status/has_committed_qc/validator_count

**修改文件**：`state_machine.rs`, `orchestrator.rs`, `mobile_bridge.rs`, `tx_bridge.rs`, `mobile_packet.rs`, `main.rs`, `adapter.rs`, `rpc.rs`, 以及两个集成测试

### Phase 6：可观测性 + Keystore + 对等评分（I2, B8, I7）

**设计决策**：
- 使用 `metrics = "0.24"` crate（与 reth 一致），8 个关键指标：
  - `n42_blocks_committed_total` (counter)
  - `n42_consensus_view` (gauge)
  - `n42_view_timeouts_total` (counter)
  - `n42_equivocations_detected_total` (counter)
  - `n42_sync_requests_served_total` (counter)
  - `n42_connected_peers` (gauge)
  - `n42_mobile_connected_phones` (gauge)
  - `n42_consensus_messages_received` (counter)
- Keystore：scrypt(N=8192) + AES-256-GCM 加密存储 BLS 私钥
  - 优先级：`N42_KEYSTORE_PATH` > `N42_VALIDATOR_KEY`（警告） > 随机（dev）
- GossipSub 对等评分：per-topic 权重，invalid_message 负分，first_delivery 正分
  - 阈值：gossip=-50, publish=-100, graylist=-200

**修改文件**：`Cargo.toml`(workspace+n42-node+n42-network+n42-node-bin), `orchestrator.rs`, `mobile_bridge.rs`, `transport.rs`, `main.rs`, 新增 `keystore.rs`

### 验证结果

- `cargo check --bin n42-node` — 通过
- `cargo test` 全部 **258 测试通过**（包含 3 个新 keystore 测试、更新的通道类型测试）
- 无功能回归
...
| # | 问题 | 状态 |
|---|------|------|
| B1 | 仅有 dev 配置，无文件加载 | ✅ Phase 1 |
| B2 | Epoch 转换未集成 | ✅ Phase 4 |
| B3 | State sync 缓冲区过小 | ✅ Phase 3 |
| B4 | Leader 同步块 payload 为空 | ✅ Phase 3 |
| B5 | 无对等节点发现 | ⏳ 未处理（需要 mDNS/DHT 集成） |
| B6 | 无消息版本控制 | ✅ Phase 2 |
| B7 | Chain ID 不匹配 | ✅ Phase 1 |
| B8 | 验证者私钥明文 | ✅ Phase 6 |
| I1 | 无未来 view 缓冲 | ✅ Phase 2 |
| I2 | 零 Prometheus 指标 | ✅ Phase 6 |
| I3 | O(n²) 消息 | ✅ Phase 9（定向消息基础设施） |
| I4 | attestation 未使用 | ✅ Phase 7 |
| I5 | 所有通道无界 | ✅ Phase 5 |
| I6 | Adapter QC 类型不匹配 | ✅ Phase 5 |
| I7 | 无 GossipSub 对等评分 | ✅ Phase 6 |
| I8 | 无健康检查端点 | ✅ Phase 5 |
| I9 | View 同步增强 | ✅ Phase 9（metrics 追踪） |
| I10 | 问责证据无持久化 | ✅ Phase 8 |
| I12 | 创世分配为空 | ✅ Phase 7 |
| I13 | 空块优化 | ✅ Phase 9 |
| I14 | 失败 attestation 无处理 | ✅ Phase 7 |
| I16 | 配置无文档 | ✅ Phase 8 |

---

## Phase 7: 创世分配 + Attestation 闭环 (2026-02-17)

### 设计决策

1. **创世分配 (I12)**：新增 `n42_dev_chainspec_with_alloc()` 和 `n42_chainspec_from_genesis()` 两个函数，保持原 `n42_dev_chainspec()` 不变以兼容。dev 模式自动分配 10,000 ETH 给验证者和 10 个测试账户。
2. **Attestation 存储 (I4)**：在 `SharedConsensusState` 中添加 `attestation_history` (VecDeque, 上限 1000) 和 `equivocation_log` (Vec, 上限 500)。RPC 暴露 `n42_blockAttestation`、`n42_attestationStats`、`n42_equivocations` 三个新方法。
3. **失败 Attestation (I14)**：`process_receipt()` 中通过 `receipt.is_valid()` 区分有效/无效收据，添加 `n42_mobile_valid_receipts_total` 和 `n42_mobile_invalid_receipts_total` 计数器。当同一区块的无效收据 ≥ connected_count/3 时发出 CRITICAL 级别告警。

### 修改文件

- `crates/n42-chainspec/src/lib.rs` — 新增 `n42_dev_chainspec_with_alloc`, `n42_chainspec_from_genesis`, `ten_thousand_eth`
- `crates/n42-node/src/consensus_state.rs` — 新增 `AttestationRecord`, `EquivocationEvidence`, 相关方法
- `crates/n42-node/src/rpc.rs` — 新增 `blockAttestation`, `attestationStats`, `equivocations` RPC
- `crates/n42-node/src/mobile_bridge.rs` — invalid receipt 追踪和 divergence 告警
- `bin/n42-node/src/main.rs` — attestation logger 写入 SharedConsensusState

### 测试

- n42-chainspec: 17 tests (新增 4)
- n42-node: 44 tests (新增 5: attestation record/stats/equivocation/history bounded)

---

## Phase 8: 节点发现 + 问责证据 + 配置文档 (2026-02-17)

### 设计决策

1. **mDNS (B5)**：使用 `libp2p::swarm::behaviour::toggle::Toggle` 包装 mDNS，可在运行时通过 `N42_ENABLE_MDNS=true` 启用。生产环境默认禁用。发现 peer 时自动 dial + 添加到 GossipSub explicit peers。
2. **问责证据 (I10)**：在 `orchestrator.rs` 的 `EquivocationDetected` 分支调用 `state.record_equivocation()`，证据存储在 `SharedConsensusState`，通过 `n42_equivocations` RPC 可查。
3. **配置文档 (I16)**：创建 `config.example.toml`，列出所有环境变量及默认值。

### 修改文件

- `Cargo.toml` — 添加 libp2p `mdns` feature
- `crates/n42-network/src/transport.rs` — N42Behaviour 添加 `mdns: Toggle<mdns::Behaviour>`, TransportConfig 添加 `enable_mdns`
- `crates/n42-network/src/service.rs` — 处理 mDNS Discovered/Expired 事件
- `crates/n42-node/src/orchestrator.rs` — 问责证据保存
- `config.example.toml` — 新文件

---

## Phase 9: 定向消息 + 空块优化 + View 同步 (2026-02-17)

### 设计决策

1. **定向消息 (I3)**：
   - Identify agent_version 携带 validator index (`n42/1.0.0/v{index}`)
   - `NetworkService` 在 Identify Received 事件中解析 index → PeerId 映射
   - `NetworkHandle` 共享 `validator_peer_map: Arc<RwLock<HashMap<u32, PeerId>>>`
   - 新增 `send_direct()` 方法和 `SendDirect` 命令
   - `orchestrator.rs` 的 `SendToValidator` 先尝试定向发送，失败回退广播
   - 添加 `n42_direct_messages_sent` 和 `n42_broadcast_messages_sent` 指标
   - 当前定向发送仍通过 GossipSub 实现（保持协议兼容），后续可改为 request-response 真 P2P

2. **空块跳过 (I13)**：
   - `schedule_payload_build()` 检查最近 3 个 committed blocks 的 payload 大小
   - payload < 512 bytes 视为空块
   - 连续跳过上限 `MAX_CONSECUTIVE_EMPTY_SKIPS = 3`（24 秒），之后仍出块保持 liveness
   - 添加 `n42_empty_block_skips_total` 指标

3. **View 同步 (I9)**：
   - 不引入新协议（现有 future message buffer + timeout→TC→NewView + SyncRequired 已覆盖）
   - 添加 `n42_view_changes_total` 指标用于监控 catch-up 频率

### 修改文件

- `crates/n42-network/src/transport.rs` — `build_swarm_with_validator_index`, agent_version 携带 index
- `crates/n42-network/src/service.rs` — `validator_peer_map`, `SendDirect` command, Identify 解析
- `crates/n42-network/src/lib.rs` — 导出 `build_swarm_with_validator_index`
- `crates/n42-node/src/orchestrator.rs` — 定向发送逻辑, 空块跳过, view 同步指标
- `bin/n42-node/src/main.rs` — 使用 `build_swarm_with_validator_index`

### 测试

全量 337 tests 通过, 0 failures。
...
**Phase 1-9 共计解决 24 项问题，337 tests passing。**

---

## P0 修复：4 项生产阻塞问题 (2026-02-17)

### 背景

在 8 模块生产就绪度审查中识别出 4 项 P0 阻塞问题，经深入分析后重新评估优先级：

- **Issue 1** (P1): Execution 类型缺少 serde — `ExecutionWitness`/`CompactWitness`/`StateDiff` 等内部类型无法序列化
- **Issue 2** (P0): DHT 节点发现 — 公网部署无运行时对等发现，依赖手动配置
- **Issue 3** (P2): 共识持久化缺少版本号和校验 — 无法检测格式变更或数据不一致
- **Issue 4** (P0，最关键): 断连节点永不重拨 — 网络必然退化

### Issue 1: Execution 类型序列化

**设计决策：**
- 仅为内部类型 (`ExecutionWitness`, `CompactWitness`, `StateDiff`, `AccountDiff`, `AccountChangeType`, `ValueChange<T>`) 添加 serde derives
- 不改 reth 的 `BlockExecutionOutput`（来自上游，无 serde 支持）
- 启用 `reth-trie-common` 的 `serde` feature 以支持 `HashedPostState` 序列化

**修改文件：**
- `Cargo.toml` — `reth-trie-common` 添加 `features = ["serde"]`
- `crates/n42-execution/Cargo.toml` — dev-dependencies 添加 serde_json, bincode
- `crates/n42-execution/src/witness.rs` — 两个 struct 添加 `serde::Serialize, serde::Deserialize` + 4 个 roundtrip 测试
- `crates/n42-execution/src/state_diff.rs` — 4 个类型添加 serde derives + 3 个 roundtrip 测试

### Issue 2: Kademlia DHT 节点发现

**设计决策：**
- 使用 `libp2p-kad` 的 MemoryStore（无需持久化 DHT 路由表）
- 通过 `Toggle` 包装，默认禁用，通过 `N42_ENABLE_DHT=true` 环境变量启用
- 自定义协议名 `/n42/kad/1.0.0` 防止与其他 libp2p 网络冲突
- query timeout 60s，路由表每 5 分钟用 random PeerId 刷新
- Identify 收到地址时自动注册到 Kademlia
- 发现新 peer 时自动拨号并注册到 ReconnectionManager（非 trusted）

**修改文件：**
- `Cargo.toml` — libp2p features 添加 `"kad"`
- `crates/n42-network/src/transport.rs` — `N42Behaviour` 添加 `kademlia: Toggle<kad::Behaviour>`, `TransportConfig` 添加 `enable_kademlia`
- `crates/n42-network/src/service.rs` — `AddKademliaPeer` command, DHT refresh 定时器, Kademlia 事件处理
- `bin/n42-node/src/main.rs` — 读取 `N42_ENABLE_DHT` 环境变量, trusted peers 注册到 Kademlia
- `config.example.toml` — `N42_ENABLE_DHT` 文档

### Issue 3: 共识持久化增强

**设计决策：**
- 添加 `version: u32` 字段（`serde(default = "default_version")` 保证向后兼容）
- 加载时对未知版本输出 warning 但不拒绝（允许降级节点启动）
- `validate()` 方法检查：`locked_qc.view <= current_view` 且 `last_committed_qc.view <= locked_qc.view`
- 校验失败只 warn 不阻止加载（宁可启动不一致也不拒绝启动）

**修改文件：**
- `crates/n42-node/src/persistence.rs` — 添加 version 字段、validate() 方法、加载校验
- `crates/n42-node/src/orchestrator.rs` — 两处 ConsensusSnapshot 构造添加 `version: 1`

### Issue 4: 自动重连 + 指数退避

**设计决策：**
- `ReconnectionManager` 独立模块，不耦合 swarm 实现
- 退避算法：`min(current * 2 * jitter(±20%), max_backoff)` — jitter 避免重连风暴
- Trusted peers 永远重试（initial: 5s, max: 300s）
- Discovered peers 最多重试 10 次后放弃
- 每 5 秒检查一次待重连列表
- ConnectionEstablished 重置退避, ConnectionClosed 标记待重连, OutgoingConnectionError 增加退避
- Rust 2024 edition 中 `gen` 是保留关键字，需使用 `r#gen` 调用 rand

**修改文件：**
- 新建 `crates/n42-network/src/reconnection.rs` — ReconnectionManager + 6 个单元测试
- `crates/n42-network/src/lib.rs` — 添加 `pub mod reconnection`
- `crates/n42-network/Cargo.toml` — 添加 `rand.workspace = true`
- `crates/n42-network/src/service.rs` — `RegisterPeer` command, 重连定时器, swarm event 回调
- `bin/n42-node/src/main.rs` — trusted peers 拨号后注册到 ReconnectionManager
...
| `cargo test -p n42-execution` (30 tests) | ✅ 全部通过 |
| `cargo test -p n42-node persistence` (8 tests) | ✅ 全部通过 |
| `cargo test -p n42-network reconnection` (6 tests) | ✅ 全部通过 |
| `cargo test -p n42-network transport` (5 tests) | ✅ 全部通过 |

### 遇到的问题

1. **Rust 2024 `gen` 保留关键字**：`rand::Rng::gen()` 需要写成 `r#gen()`，因为 edition 2024 将 `gen` 保留为未来协程语法
2. **Legacy snapshot JSON 格式**：测试中手写的 QC JSON 不匹配实际 `BlsSignature` 序列化格式，改为代码生成 → 删除 version 字段的方式

### 新增测试清单

- `test_execution_witness_serde_json_roundtrip`
- `test_execution_witness_bincode_roundtrip`
- `test_compact_witness_serde_json_roundtrip`
- `test_compact_witness_bincode_roundtrip`
- `test_state_diff_serde_json_roundtrip`
- `test_state_diff_bincode_roundtrip`
- `test_account_change_type_serde_roundtrip`
- `test_load_legacy_snapshot_without_version`
- `test_validate_valid_snapshot`
- `test_validate_locked_qc_exceeds_current_view`
- `test_validate_committed_exceeds_locked`
- `test_backoff_increases`
- `test_connection_resets_backoff`
- `test_trusted_never_abandoned`
- `test_discovered_stops_after_max_attempts`
- `test_jitter_range`
- `test_connected_peer_not_reconnected`
