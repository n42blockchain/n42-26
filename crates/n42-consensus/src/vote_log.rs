...
//! durable record of `last_voted_view` immediately *before* signing each R1
//! vote. The orchestrator owns the file handle so the engine itself stays
...
/// A sink that durably records the highest view this validator has signed an
/// R1 vote in. Implementations must `fsync` (or equivalent) before returning
/// `Ok(())`; on `Err`, the engine aborts the vote.
