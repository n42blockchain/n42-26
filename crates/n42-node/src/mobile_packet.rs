        ...
        + 'static,
{
    ...
    while let Some((block_hash, block_number)) = rx.recv().await {
...
fn fix_witness_pre_state(
    ...
    for (idx, wa) in packet.witness_accounts.iter_mut().enumerate() {
        ...
        for (_slot_key, value) in &mut wa.storage {
            let slot_b256 = B256::from(_slot_key.to_be_bytes());
