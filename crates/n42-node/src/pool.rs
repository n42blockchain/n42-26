...
use reth_transaction_pool::{
    blobstore::NoopBlobStore, CoinbaseTipOrdering, EthPooledTransaction,
    EthTransactionValidator, PoolConfig, SubPoolLimit, TransactionValidationTaskExecutor,
};

/// N42 交易池类型别名 — 使用 NoopBlobStore 替代 DiskFileBlobStore
...
/// - 禁用 EIP-4844 blob 交易（NoopBlobStore）
    ...
    async fn build_pool(
        ...
        let validator =
            TransactionValidationTaskExecutor::eth_builder(ctx.provider().clone(), evm_config)
                ...
                .build_with_tasks(ctx.task_executor().clone(), NoopBlobStore::default());
        ...
        let pool = TxPoolBuilder::new(ctx)
            ...
            .build_and_spawn_maintenance_task(NoopBlobStore::default(), pool_config)?;
...
fn idc_pool_config(base: &PoolConfig) -> PoolConfig {
    PoolConfig {
        ...
        blob_limit: SubPoolLimit { max_txs: 0, max_size: 0 },
