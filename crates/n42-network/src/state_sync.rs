...
impl request_response::Codec for StateSyncCodec {
    ...
    where
        ...
        Self: 'async_trait,
    {
        Box::pin(codec::read_length_prefixed(io, MAX_SYNC_MESSAGE_SIZE))
    }
    ...
    where
        ...
        Self: 'async_trait,
    {
        Box::pin(async move { codec::write_length_prefixed(io, &req, MAX_SYNC_MESSAGE_SIZE).await })
    }
    ...
    where
        ...
        Self: 'async_trait,
    {
        Box::pin(async move { codec::write_length_prefixed(io, &res, MAX_SYNC_MESSAGE_SIZE).await })
    }
}
...
mod tests {
    ...
    async fn test_codec_write_read_request_roundtrip() {
        ...
        codec::write_length_prefixed(&mut buf, &req, MAX_SYNC_MESSAGE_SIZE)
            .await
            .unwrap();
        ...
        let decoded: BlockSyncRequest =
            codec::read_length_prefixed(&mut reader, MAX_SYNC_MESSAGE_SIZE)
                .await
                .unwrap();
        assert_eq!(decoded.from_view, req.from_view);
        assert_eq!(decoded.to_view, req.to_view);
    ...
    async fn test_codec_write_read_response_roundtrip() {
        let resp = sample_response();
        let mut buf = futures::io::Cursor::new(Vec::new());
        codec::write_length_prefixed(&mut buf, &resp, MAX_SYNC_MESSAGE_SIZE)
            .await
            .unwrap();
    ...
    async fn test_codec_read_rejects_oversized() {
        let fake_len = (MAX_SYNC_MESSAGE_SIZE as u32 + 1).to_be_bytes();
        ...
        let result: io::Result<BlockSyncRequest> =
            codec::read_length_prefixed(&mut reader, MAX_SYNC_MESSAGE_SIZE).await;
