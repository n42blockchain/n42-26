...
impl request_response::Codec for ConsensusDirectCodec {
    ...
    {
        codec::read_length_prefixed(io, MAX_CONSENSUS_DIRECT_SIZE).await
    }
    ...
    {
        codec::write_length_prefixed(io, &req, MAX_CONSENSUS_DIRECT_SIZE).await
    }
    ...
    where
        T: AsyncWrite + Unpin + Send,
    {
        codec::write_length_prefixed(io, &res, MAX_CONSENSUS_DIRECT_SIZE).await
    }
}
