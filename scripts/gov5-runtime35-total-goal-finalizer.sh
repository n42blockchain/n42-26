...
runtime="${N42_TOTAL_RUNTIME:-/Users/jieliu/Documents/n42/live-interop-20260721/runtime-35-gov5-906-rustsec-final}"
...
expected_gov_main="75dab6e5d1bc6aefa213f4f0f7dcc972dd04f89d"
expected_gov_candidate="23533225648c62440c6112f88dfdb64b5f55f3f1"
expected_n42="ce4e88ccfe7bc845ecd57605d417a7559fbde932"
expected_reth="0fc810bae34412838bedfd8dc2f212e14e915e5d"
expected_gov_binary="e08c1ea7aac198e268d7fb07eacce33347798c90f33532ffc4cd85bfc1af7033"
...
expected_static_v3="8ce579852659a4c696cc3f2131df6bc239f6caaede0ffc4aab67bb6e292dc37f"
...
strict="$runtime/evidence/gov5-906-final-qualification.json"
independent="$runtime/evidence/gov5-906-independent-final-verification.json"
producer="$runtime/evidence/runtime35-rustsec-final-strict24h-six-producer.json"
...
stable="$runtime/evidence/official-reth-stable-monitor.jsonl"
copied="$runtime/evidence/runtime35-stopped-copy-manifest.json"
...
failure="${N42_TOTAL_FAILURE:-$runtime/evidence/runtime35-rustsec-final-goal-completion-failure.json}"
...
assert_no_failures() {
  local item
  for item in \
    "$runtime/evidence/gov5-current-main-fail-close-guardian-failure.json" \
    "$runtime/evidence/gov5-906-finalizer-failures.jsonl" \
    "$runtime/evidence/gov5-906-independent-final-verification-failure.json" \
    "$runtime/evidence/runtime28-strict24h-six-producer-full-range-failure.json"; do
    test ! -s "$item"
  done
}

assert_live() {
  local expected_nonce="${1:?expected nonce is required}"
  local port genesis boundary security nonce pending expected identity exact attempt
  for port in "${ports[@]}"; do
    genesis="$(rpc "$port" eth_getBlockByNumber '["0x0",false]' | jq -er '.result.hash')"
    boundary="$(rpc "$port" eth_getBlockByNumber '["0x169bd",false]' | jq -er '.result.hash')"
    security="$(rpc "$port" eth_getBlockByNumber '["0x18637",false]' | jq -er '.result.hash')"
    ...
    test "$boundary" = "$expected_905_boundary"
    test "$security" = "$expected_security_boundary"
  ...
  test "$exact" = true
  rpc 29545 n42_consensusStatus '[]' |
    jq -e '.result.validatorCount==7 and .result.hasCommittedQc==true' >/dev/null
  rpc 29545 n42_equivocations '[]' |
    jq -e '.result.total==0 and (.result.evidence|length)==0' >/dev/null
}
...
assert_static() {
  ...
  test "$(sha256 "$runtime/artifacts/scripts/gov5-current-qualification-finalizer.sh")" = \
    "$expected_finalizer"
  test "$(sha256 "$static_v3")" = "$expected_static_v3"
  "$runtime/n42-node" --version | rg -F 'Reth Version: 2.4.1' >/dev/null
  "$runtime/n42-node" --version | rg -F "Commit SHA: $expected_reth" >/dev/null
  jq -e --arg gov "$expected_gov_main" --arg candidate "$expected_gov_candidate" \
    --arg n42 "$expected_n42" --arg reth "$expected_reth" \
    --arg gov_binary "$expected_gov_binary" --arg rust_binary "$expected_rust_binary" '
    .status=="PASS" and .sources.gov5Version=="5.7.906" and
    .sources.gov5Main==$gov and .sources.gov5InteropCandidate==$candidate and
    .sources.n42==$n42 and .sources.reth==$reth and
    .binaries.gov5Sha256==$gov_binary and .binaries.rustSha256==$rust_binary and
    .chainBoundaries.genesis.hash=="0xb71c28109836f120453d097c38819a55b14c49abcc92713037fb9b11201392ec" and
    .chainBoundaries.gov5_905CopiedBoundary.height==92605 and
    .chainBoundaries.gov5_905CopiedBoundary.hash=="0xb88a3571223cf8cd8291d608572a55f306ea88957cc7ede8ab6b8812ada85a82" and
    .chainBoundaries.securityRefreshSwitchBoundary.height==99895 and
    .chainBoundaries.firstFormalRustProducerHeight==99926
  ' "$launch" >/dev/null
  jq -e '.status=="PASS" and .files==141 and (.entries|length)==141 and
    .allPathsSizesAndHashesExact==true and
    .sourceManifestSha256==.targetManifestSha256' "$copied" >/dev/null
  jq -e '.status=="PASS" and .staticGov5Data.filesChecked==24 and
    .staticGov5Data.allCurrentHashesMatchInitialCopy==true and
    .frozenTools.independentVerifierSha256=="fa0f06dbadd23e2c662ac2790edbbad5405b0297c424c051bcc096bd61084529" and
    .binaries.gov5Sha256=="e08c1ea7aac198e268d7fb07eacce33347798c90f33532ffc4cd85bfc1af7033" and
    .binaries.rustSha256=="d639f712a87c22c2a45de29dbd895897058a8a28e4a2145061bd195d79eb6d2e"' \
    "$static_v3" >/dev/null
  jq -e '.status=="PASS" and .chainOrDataMutation==false and
    .currentTotalGoalVerifierPredatesFormalStart==true and .v2Preserved==true and
  ...
  jq -e '.status=="PASS" and .rustClientVersionExact==true and
    .consensusNetworkConnectedAndQuorate==true and
    .allSixCommittedBlockIdentityExact==true and .equivocations.total==0' \
    "$network" >/dev/null
  assert_sources
}
...
if test "$preflight_only" = 1; then
  jq -nc --arg at "$(date -u +%FT%TZ)" --arg self "$expected_self_sha" \
    --arg interop "$expected_interop" \
    '{at:$at,event:"runtime35_total_goal_finalizer_preflight",status:"PASS",
      scriptSha256:$self,interopCommit:$interop,nodesAlive:true,binariesExact:true,
      sourcesAndRemotesExact:true,genesis905AndSecurityBoundariesExact:true,
      staticBoundaryV3Exact:true,liveSixEndpointIdentityExact:true,
      zeroEquivocations:true,officialRethStable:"v2.4.1",transactionsSent:0}'
  exit 0
fi
...
on_error() {
  ...
  jq -nc --arg at "$(date -u +%FT%TZ)" --argjson status "$status" \
    ...
    '{at:$at,event:"runtime35_total_goal_finalizer_failure",status:"FAIL",
...
milestone_files=()
for spec in 'formal-15m 900' 'formal-1h 3600' 'formal-4h 14400' 'formal-6h 21600'; do
  label="${spec%% *}"
  milestone_files+=(
    "$runtime/evidence/gov5-906-$label-milestone.json"
    "$runtime/evidence/runtime35-rustsec-final-$label-supplemental-audit.json"
    "$runtime/evidence/runtime35-$label-closed-deep-audit.json"
    "$runtime/evidence/runtime35-rustsec-final-$label-six-producer.json"
  )
done

required=("$strict" "$independent" "$producer" "$producer_linkage")
required+=("${milestone_files[@]}")
while :; do
  missing=false
  for item in "${required[@]}"; do
    test -s "$item" || missing=true
  done
...
jq -e '.status=="PASS" and .transactionsFinalized==17 and
  ...
  .liveArchiveParityReexecuted==true' "$independent" >/dev/null
jq -e '.status=="PASS" and .startHeight==99926 and
  ...
  .zeroTransactions==true and .completeCycles>0' "$producer" >/dev/null
jq -e --arg producer_sha "$(sha256 "$producer")" '
  .status=="PASS" and .producerAuditSha256==$producer_sha and
  .historicalWindowOnly==true and
  .postSoakTransactionsCannotAlterAuditedHistory==true' "$producer_linkage" >/dev/null

jq -e -s 'length>=2 and all(.[];.ok==true and .zeroTxRequired==1 and .lag<=1) and
  ((.[-1].at|fromdateiso8601)-(.[0].at|fromdateiso8601))>=86400 and
  ([range(1;length) as $i |
    ((.[$i].at|fromdateiso8601)-(.[$i-1].at|fromdateiso8601))<=120] | all)' \
  "$formal" >/dev/null
jq -e -s --arg expected "$expected_gov_main" 'length>=2 and
  ...
  "$upstream" >/dev/null
jq -e '.status=="PASS" and .elapsedSeconds>=86400' "$upstream_complete" >/dev/null
jq -e '.status=="PASS" and .elapsedSeconds>=86400' "$upstream_audit" >/dev/null
jq -e '.status=="PASS" and .elapsedSeconds>=86400 and .singleProcess==true and
  .logicalCountersMonotonic==true and .headLogAndWalCountersMonotonic==true' \
...
for spec in 'formal-15m 900' 'formal-1h 3600' 'formal-4h 14400' 'formal-6h 21600'; do
  ...
  jq -e --arg label "$label" --argjson minimum "$minimum" '
    ...
    .soak.zeroTransactionRequired==true and .transactionsSent==0 and
    .equivocations.total==0' \
    "$runtime/evidence/gov5-906-$label-milestone.json" >/dev/null
  jq -e '.status=="PASS" and .archiveAndQmdbParityExact==true and
    .networkConsensusExact==true and .data905CompatibilityExact==true and
    .resourceTrendWithin24hBudget==true and .noFailureEvidence==true' \
    "$runtime/evidence/runtime35-rustsec-final-$label-supplemental-audit.json" >/dev/null
  jq -e '.status=="PASS" and .rustLeaders.leaderCommitLog.allVotesFivePlusFive==true and
    .timeoutRecovery.pendingTimeouts==0 and .runtimeLogs.unexpectedWarnings==0 and
    .runtimeLogs.criticalSignals==0 and
    .staticBoundary.staticGov5Data.allCurrentHashesMatchInitialCopy==true' \
    "$runtime/evidence/runtime35-$label-closed-deep-audit.json" >/dev/null
  jq -e '.status=="PASS" and .startHeight==99926 and
    .allSixEndpointSequencesExact==true and .parentChainContinuous==true and
    .expectedProducerSlotsExact==true and .allProducerCountsBalanced==true and
    .zeroTransactions==true' \
    "$runtime/evidence/runtime35-rustsec-final-$label-six-producer.json" >/dev/null
done

audit_dir="$(mktemp -d /tmp/n42-runtime35-total.XXXXXX)"
trap 'rm -rf "$audit_dir"' EXIT
"$rechecker" "$runtime" "$static_v3" "$audit_dir/static.json" >/dev/null
jq -e '.status=="PASS" and .staticGov5Data.filesChecked==24 and
  .staticGov5Data.allCurrentHashesMatchInitialCopy==true' "$audit_dir/static.json" >/dev/null
"$data_auditor" "$runtime" "$gov_repo" "$expected_gov_main" \
  "$audit_dir/data905.json" 0x22 >/dev/null
jq -e '.status=="PASS" and .latestAndPendingNonce=="0x22" and
  .dataRecopyOrRegenerationRequired==false' "$audit_dir/data905.json" >/dev/null
N42_NETWORK_EXPECTED_RUST_CLIENT="$expected_client" \
  "$network_auditor" "$runtime" "$audit_dir/network.json" >/dev/null
jq -e '.status=="PASS" and .allSixCommittedBlockIdentityExact==true and
  .consensusNetworkConnectedAndQuorate==true and .equivocations.total==0' \
  "$audit_dir/network.json" >/dev/null
...
env N42_QUAL_RUNTIME="$runtime" N42_VERIFY_REPO="$n42_repo" \
  ...
  N42_VERIFY_EXPECTED_SELF_SHA="$expected_verifier" \
  N42_VERIFY_DEPS_HEAD="$expected_n42" N42_VERIFY_RETH_HEAD="$expected_reth" \
  N42_VERIFY_GOV_BINARY_SHA="$expected_gov_binary" \
  N42_VERIFY_RUST_BINARY_SHA="$expected_rust_binary" \
  "$verifier" >"$audit_dir/independent.json"
jq -e '.status=="PASS" and .transactionsFinalized==17' \
  "$audit_dir/independent.json" >/dev/null
...
head_stats="$(jq -cs '{samples:length,firstAt:.[0].at,lastAt:.[-1].at,
  ...
  maximumLag:([.[].lag]|max),failures:([.[]|select(.ok!=true)]|length)}' "$formal")"
latest_head="$(rpc 29545 eth_getBlockByNumber '["latest",false]' |
  jq -ec '.result|{number,hash,stateRoot,receiptsRoot,transactionsRoot}')"
consensus="$(rpc 29545 n42_consensusStatus '[]' | jq -ec '.result')"
equivocations="$(rpc 29545 n42_equivocations '[]' | jq -ec '.result')"
temporary="$(mktemp "$runtime/evidence/.runtime35-total.XXXXXX")"
jq -nc --arg at "$(date -u +%FT%TZ)" --arg self "$expected_self_sha" \
  ...
  --arg n42 "$expected_n42" --arg reth "$expected_reth" --arg interop "$expected_interop" \
  --arg strict_sha "$(sha256 "$strict")" \
  ...
  '{at:$at,event:"runtime35_rustsec_final_goal_completion",status:"PASS",
    ...
    verifierScriptSha256:$self,strict24h:$strict,finalCanonicalHead:$head,
    consensus:$consensus,equivocations:$equivocations,
    genesis905AndSecurityBoundariesExact:true,copiedDataExact:true,
    allSixEndpointsExact:true,strict24hZeroTransactionExact:true,
...
jq -e '.status=="PASS" and .strict24h.elapsedSeconds>=86400 and
  ...
  .sixProducerRotationExact==true and .controlledRustRestartRejoined==true and
  .sourceAndRemotePinsExact==true and .noFailureEvidence==true' "$temporary" >/dev/null
mv "$temporary" "$output"
trap - EXIT
rm -rf "$audit_dir"
cat "$output"
