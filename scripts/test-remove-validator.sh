...
if [[ $height_diff -le 3 && $min_h -gt 0 ]]; then
    echo -e "  ${GREEN}✓${NC} 剩余节点高度一致: 最大差距 ${height_diff} 块（${min_h} ~ ${max_h}）"
else
    echo -e "  ${RED}✗${NC} 剩余节点高度不一致: 差距 ${height_diff} 块（${min_h} ~ ${max_h}）"
    PASS=false
fi
...
if [[ $max_h -gt $activate_block ]]; then
    echo -e "  ${GREEN}✓${NC} epoch 切换后持续出块: $activate_block → $max_h"
else
