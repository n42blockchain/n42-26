#!/usr/bin/env bash
#
# 42-node 10-minute stress test with 8s block interval
...
DATA_DIR="$HOME/n42-testnet-data-42"
BASE_HTTP_RPC=18545
...
log()  { echo -e "${GREEN}[42node-test]${NC} $*"; }
warn() { echo -e "${YELLOW}[42node-test]${NC} $*"; }
err()  { echo -e "${RED}[42node-test]${NC} $*"; }
info() { echo -e "${CYAN}[42node-test]${NC} $*"; }
...
TESTNET_LOG="$HOME/testnet-42node.log"

# Start testnet in background
log "Starting 42-node testnet (8s block interval)..."
...
cleanup() {
    ...
    # Also kill any remaining n42-node processes from this test
    pkill -f "n42-testnet-data-42" 2>/dev/null || true
...
STARTUP_WAIT=0
MAX_STARTUP_WAIT=420  # 7 minutes max for 42 nodes to start (startup_delay ~102s + buffer)
...
log "Recording initial block numbers..."
INITIAL_BLOCKS=()
...
log "Initial blocks recorded (v0: ${INITIAL_BLOCKS[0]})"
...
while [[ $ELAPSED -lt $TEST_DURATION ]]; do
    ...
    if [[ $((ELAPSED - LAST_REPORT)) -ge $REPORT_INTERVAL ]]; then
        LAST_REPORT=$ELAPSED
        # Sample block numbers from a few nodes
        ...
        blocks_v20=$(curl -s --max-time 3 \
            -X POST -H 'Content-Type: application/json' \
            -d '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' \
            "http://127.0.0.1:$((BASE_HTTP_RPC + 20))/" 2>/dev/null | \
            python3 -c "import sys,json; print(int(json.load(sys.stdin).get('result','0x0'),16))" 2>/dev/null || echo "?")

        blocks_v41=$(curl -s --max-time 3 \
            -X POST -H 'Content-Type: application/json' \
            -d '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' \
            "http://127.0.0.1:$((BASE_HTTP_RPC + 41))/" 2>/dev/null | \
            python3 -c "import sys,json; print(int(json.load(sys.stdin).get('result','0x0'),16))" 2>/dev/null || echo "?")

        remaining=$((TEST_DURATION - ELAPSED))
        info "[${ELAPSED}s/${TEST_DURATION}s] Blocks: v0=$blocks_v0 v20=$blocks_v20 v41=$blocks_v41 (${remaining}s remaining)"
    fi
done
...
for i in $(seq 0 $((NUM_VALIDATORS - 1))); do
    ...
    if [[ -f "$logfile" ]]; then
        errors=$(grep -ci "error\|panic\|fatal" "$logfile" 2>/dev/null || echo "0")
...
ALIVE=0
for i in $(seq 0 $((NUM_VALIDATORS - 1))); do
    # Check if process is alive by trying RPC
    b=$(curl -s --max-time 2 \
        -X POST -H 'Content-Type: application/json' \
        -d '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' \
...
echo ""
log "========================================="
log "  42-NODE TEST RESULTS (10 minutes, 8s slots)"
log "========================================="
echo ""
...
if [[ $RESPONSIVE -gt 0 ]]; then
    ...
    # Check if block production rate is reasonable (at least 50% of expected)
